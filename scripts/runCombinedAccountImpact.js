...
function simulateYear(m15CandlesBySymbolFull, divergenceCandidatesFull, year) {
  ...
  if (m15BySymbol.US100.length < 50 && m15BySymbol.US500.length < 50) return null;
  ...
  const openFvg = { US100: null, US500: null };
  let openDivergence = null; // { symbol, entryIndex, entryTime, entryPrice, stopPrice, targetPrice, distance }
  const symbolIndex = { US100: -1, US500: -1 };
  const formationIndexBySymbol = { US100: new Map(), US500: new Map() };
  ...
  const resolveClose = (symbol, netR, exitTime, riskAmount, isDivergence, outcome) => {
    ...
    totalTrades++;
    if (isDivergence) divTrades++; else fvgTrades++;
  ...
  for (const { symbol, candle } of timeline) {
    ...
    // 4) Check for a Divergence candidate entry trigger on this exact candle/symbol.
    const cand = divCandidatesByTime.get(symbol).get(candle.time);
    if (cand && !openDivergence) {
      const divSpread = DEFAULT_SPREADS[symbol] ?? 0;
...
function fmtRow(year, r) {
  ...
  return `| ${year}${TRAIN_YEARS.has(year) ? ' (train)' : ' (test)'} | ${r.trades} (${r.fvgTrades} FVG + ${r.divTrades} div.) | ${wr} | ${r.staticDrawdownPct.toFixed(1)}% | ${r.trailingDrawdownPct.toFixed(1)}% | ${bustCell} | ${r.phase1Label} | ${r.phase2Label} | $${r.finalBalance.toFixed(0)} |`;
}
...
function main() {
  ...
  const md = [];
  md.push('# Impact au niveau du COMPTE — FVG + Divergence COMBINÉS, mêmes garde-fous partagés (0.5%/trade)');
  md.push('');
  md.push(
    `⚠ Compte $${STARTING_BALANCE}, un seul budget de garde-fous PARTAGÉ entre les 3 sources de signaux (US100-FVG, ` +
      'US500-FVG, Divergence), garde-fous INCHANGÉS (max 2 trades/jour, cooldown 30min après perte, perte quotidienne ' +
      "max 2%) - pas assouplis pour laisser plus de place à la divergence. Divergence = config choisie par classement " +
      "TRAIN seul (lookback 100, seuil z=2). Simplification connue : une position Divergence et une position FVG sur " +
      "le MÊME instrument peuvent être ouvertes simultanément dans ce modèle (slots suivis indépendamment)."
  );
  md.push('');
  ...
  for (const year of YEARS) {
    ...
    console.error(`${year}: ${r ? r.trades + ' trades (' + r.fvgTrades + ' FVG + ' + r.divTrades + ' div), solde $' + r.finalBalance.toFixed(0) : 'skip'}`);
  }

  const outMd = path.join(dir, 'combined-account-impact.md');
