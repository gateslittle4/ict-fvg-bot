...
const N = 200, SYM = 'US100';
let seed = 26092026; const rnd = () => { seed = (seed * 1103515245 + 12345) % 2147483648; return seed / 2147483648; };
const S = loadPhase(SYM, 'explore'), X = buildContext(S);
const opps = opportunities(qualifications(X), { ageMin: 0, ageMax: 24, fromMin: 180, toMin: 660 }).filter((o) => o.tau >= eng(2011, 1) && o.tau < eng(2019));
...
for (const d of days) {
  ...
  const id = `s${String(setups.length + 1).padStart(3, '0')}`;
  ...
  const sp = spreadAt(SYM, S.o[i]), f0 = o.dir > 0 ? S.o[i] + sp : S.o[i], exitAt = nextNyTime(tau, 660), sw = swapCost(SYM);
...
fs.writeFileSync('data/backtest-input/blind-outcomes.json', JSON.stringify({ note: 'Résultats mécaniques des 200 cas de l\'exercice à l\'aveugle — NE PAS montrer à Esdras avant ses réponses', outcomes }));
