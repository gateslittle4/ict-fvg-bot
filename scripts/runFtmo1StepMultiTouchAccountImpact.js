...
// Usage: node scripts/runFtmo1StepMultiTouchAccountImpact.js <dir-with-csvs>
//
...
function main() {
  ...
  if (!dir) { console.error('Usage: node scripts/runFtmo1StepMultiTouchAccountImpact.js <dir-with-csvs>'); process.exit(1); }
  ...
  const md = [];
  md.push('# Combien de temps pour passer un challenge FTMO 1-Step avec ce système (US100 en multi-contact)');
  md.push('');
  md.push(
    `Question directe d'Esdras. Même scope de compte que runFtmo1StepAccountImpact.js (FVG US100+US500+XAUUSD + ` +
      `Divergence US100/US500, netting même-instrument, un seul budget de garde-fous), CORRIGÉ pour utiliser les ` +
      `RR de production réels (5/5/4, lus directement depuis CONFIG.fvg.perSymbol - l'ancien script avait 3/3/3 ` +
      `codé en dur, périmé depuis la "cible étendue"). US100 utilise \`MultiTouchFvgEngine\` (vérifié ce soir, pas ` +
      `encore déployé) ; US500/XAUUSD/Divergence restent le moteur à contact unique déjà en production.`
  );
  md.push('');
  ...
  const outMd = path.join(dir, 'ftmo-1step-multitouch-account-impact.md');
