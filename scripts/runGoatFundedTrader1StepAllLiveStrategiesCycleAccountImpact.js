#!/usr/bin/env node
// runGoatFundedTrader1StepAllLiveStrategiesCycleAccountImpact.js
// Usage: node scripts/runGoatFundedTrader1StepAllLiveStrategiesCycleAccountImpact.js <dir-with-csvs>
//
...
// Re-checking GoatFundedTrader's EA policy (tested earlier this session
// for its floating-loss rule on the Instant programs, before the
// EA-autonomy filter was known to matter) found the 1-Step evaluation
// already passes: EAs allowed fully automated "as long as they're your
// own strategy" - no assist-only restriction like Alpha Capital Group.
// This bot qualifies. Fills slot #5 (after FTMO, CTI, Ment Funding,
// FundingPips) using rules already verified in
// src/propFirms/goatFundedTrader.js (GOATFUNDEDTRADER_1STEP) - the
// EVALUATION program, not the Instant Premium/HERO programs tested
// earlier (those are instant-funded with a different, real-time-equity
// mechanism, already covered by their own dedicated scripts).
//
// Rules: target **+10%** (same as FTMO/Ment Funding), max drawdown **6%
// STATIC** (fixed below STARTING balance - same non-trailing mechanism as
// Ment Funding, confirmed against GoatFundedTrader's own help article,
// see that program's comment in goatFundedTrader.js), daily loss 3%
// (stricter default assumed - source found either 4% or 3% depending on
// purchase date, see that file's own note), minimum 3 winning days
// (≥0.5% net each) not modeled here, same as the min-trading-days
// omission in every other script in this series. Price: exact $25k figure
// not confirmed (refundable-fee pricing calculator not fully scraped -
// a $100k account was seen at ~$263 with an active promo code, so $25k
// is roughly estimated in the $60-100 range, NOT confirmed - flagged as
// uncertain in the report rather than guessed as fact).
//
...
const STATIC_MAX_LOSS_PCT = 6; // GoatFundedTrader 1-Step: fixed 6% below STARTING balance, never trails up
const CHALLENGE_TARGET_MULTIPLE = 1.10; // GoatFundedTrader 1-Step: 10% profit target
...
// Same 6 scenarios as the CTI/Ment Funding/FundingPips reports (baseline,
// 3 lower-risk variants including 0.2%, 2 concurrency-cap variants) for
// direct comparison across all five reports.
const SCENARIOS = [
...
function simulateContinuous({ candlesBySymbolFull, divergenceCandidatesFull, nwogCandidatesFull, judasCandidatesFull, riskPct, maxConcurrentPositions }) {
  ...
  const resolveClose = (symbol, netR, exitTime, riskAmount, outcome) => {
    ...
    // STATIC floor (GoatFundedTrader 1-Step): measured against the fixed STARTING
...
function main() {
  ...
  if (!dir) { console.error('Usage: node scripts/runGoatFundedTrader1StepAllLiveStrategiesCycleAccountImpact.js <dir-with-csvs>'); process.exit(1); }
  ...
  const md = [];
  md.push('# Simulation continue avec reset au +10% ou au bust (-6% statique) — GoatFundedTrader 1-Step, toutes stratégies live, compte 10k');
  md.push('');
  md.push(
    "Fork direct de ftmo-1step-..., cti-1step-..., ment-funding-1step-... et fundingpips-1step-flex-... (même " +
      "architecture de reset continu). Esdras (2026-09-12) : \"Verifie une autre platforme, on doit avoir au moins " +
      "5 pour trancher.\" En revérifiant la politique EA de **GoatFundedTrader** (déjà étudiée plus tôt dans la " +
      "session pour la règle de perte flottante des programmes Instant, avant que le filtre EA-autonome soit " +
      "connu) : le programme d'ÉVALUATION 1-Step passe déjà - EA permis pleinement automatisés \"tant que c'est " +
      "ta propre stratégie\", pas de restriction assist-only comme Alpha Capital Group. Ce bot qualifie. Complète " +
      "le slot #5 (après FTMO, CTI, Ment Funding, FundingPips) avec les règles déjà vérifiées dans " +
      "src/propFirms/goatFundedTrader.js (GOATFUNDEDTRADER_1STEP - le programme d'ÉVALUATION classique, pas les " +
      "programmes Instant Premium/HERO déjà testés séparément avec leur propre mécanisme).\n\n" +
      "**Règles** : target **+10%** (comme FTMO/Ment Funding), drawdown max **6% STATIQUE** (même mécanisme " +
      "non-trailing que Ment Funding, confirmé sur l'aide GoatFundedTrader). Perte journalière 3% (valeur la plus " +
      "stricte retenue par défaut, la source trouvait 4% ou 3% selon la date d'achat du compte - voir la note dans " +
      "goatFundedTrader.js). Minimum 3 jours gagnants (≥0.5% net chacun) non modélisé, comme partout ailleurs dans " +
      "cette série. **Prix $25k non confirmé avec certitude** - calculateur de prix non entièrement récupéré, un " +
      "compte $100k vu à ~$263 avec promo active suggère un $25k autour de $60-100, mais ce n'est PAS confirmé - " +
      "signalé comme incertain plutôt que présenté comme un fait.\n\n" +
      "**6 scénarios**, identiques aux rapports précédents pour comparaison directe ligne à ligne entre les 5 " +
      "rapports.\n\n" +
  ...
  md.push('');
  md.push(
    `Contre le plancher STATIQUE de GoatFundedTrader (6% sous le départ, jamais trailing), le scénario actuel ` +
      ...
      `plancher 10% trailing), CTI (34%, plancher 5% trailing), Ment Funding (7%, plancher 6% statique) et ` +
      `FundingPips (plancher 12% statique) pour situer GoatFundedTrader dans l'ensemble.\n\n` +
  ...
  const outMd = path.join(dir, 'goatfundedtrader-1step-all-live-strategies-cycle-account-impact.md');
