#!/usr/bin/env node
// runEqualHighsLowsStrategyAnalysis.js
// Usage: node scripts/runEqualHighsLowsStrategyAnalysis.js <dir-with-csvs>
//
// ICT "Equal Highs / Equal Lows" (EQH/EQL) liquidity sweep — see
// src/backtest/equalHighsLows.js for the full method writeup: two or more
// swing points sitting within 0.1% of each other are treated as a stronger
// liquidity pool than a single extreme (stacked resting stops), traded via
// the same wick-then-reclaim mechanic already used by liquiditySweep.js/
// Judas Swing/Weekly Sweep. Entry one candle after the sweep, stop beyond
// the sweep's extreme, fixed 1:3 R:R, 480 M15-candle timeout.
//
...
import { runEqualHighsLowsBacktest } from '../src/backtest/equalHighsLows.js';
...
function main() {
  ...
  if (!dir) {
    console.error('Usage: node scripts/runEqualHighsLowsStrategyAnalysis.js <dir-with-csvs>');
    process.exit(1);
  }

  const md = [];
  md.push('# Stratégie exploratoire ICT #12 : Equal Highs / Equal Lows (EQH/EQL) liquidity sweep');
  md.push('');
  md.push(
    "⚠ Distinct du Judas Swing / Weekly Liquidity Sweep déjà validés (ceux-ci balayent l'unique extrême le plus " +
      "récent) : ici il faut DEUX pivots de swing (haut ou bas) situés à moins de 0.1% l'un de l'autre - la doctrine " +
      "ICT veut que ce double niveau concentre plus de stops resting qu'un extrême unique. Réutilise la détection " +
      "de swing déjà existante (marketStructure.js, lookback=5). Signal au balayage-puis-reclaim du niveau le plus " +
      "récent des deux, entrée une bougie après confirmation, stop au-delà de l'extrême du balayage, cible fixe " +
      "1:3, timeout 480 bougies M15 (mêmes conventions que partout ailleurs). Testé sur les 12 instruments " +
      "disponibles. Écran TRAIN (avant 2024-01-01) / vérification TEST (2024-2025), même règle de verdict que " +
      "partout ailleurs."
  );
  md.push('');
  ...
  for (const symbol of SYMBOLS) {
    ...
    const trainTrades = runEqualHighsLowsBacktest(trainCandles);
    const testTrades = runEqualHighsLowsBacktest(testCandles);
  ...
  const outMd = path.join(dir, 'equal-highs-lows-strategy-analysis.md');
