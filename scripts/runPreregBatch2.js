#!/usr/bin/env node
// runPreregBatch2.js
// Usage:  node --max-old-space-size=4096 scripts/runPreregBatch2.js train
//         node --max-old-space-size=4096 scripts/runPreregBatch2.js test     (UNE seule fois, apres avoir commite le resultat de `train`)
// Applique EXACTEMENT `data/backtest-input/preregistration-batch2-2026-09-21.md`. Ne pas y changer un critere.
...
const RETAINED_FILE = 'data/backtest-input/prereg-batch2-train-retained.json';
...
// ---- the 12 pre-registered hypotheses of batch 2 (config = the coded block, symbols list replaced, nothing tuned)
const HYP = [];
const MECH = [
  ['Silver Bullet', 'silverBulletConfig', () => CONFIG.silverBullet],
  ['CBDR', 'cbdrConfig', () => CONFIG.cbdr],
  ['Judas Swing', 'judasSwingConfig', () => CONFIG.judasSwing],
  ['Breaker Block', 'breakerBlockConfig', () => CONFIG.breakerBlock],
];
for (const [label, key, get] of MECH) for (const s of NEW) HYP.push({ id: `B-${key}-${s}`, label: `${label} ${s}`, symbols: [s], make: () => ({ [key]: { ...get(), symbols: [s] } }) });
...
const md = [`# Pré-enregistrement « nouveaux instruments » — ${MODE === 'train' ? 'ENTRAÎNEMENT (avant 2025-01-01)' : 'TEST (2025-01-01 → fin), lecture unique des hypothèses retenues'}`, '', 'Règles (LOT 2, critères durcis) : `data/backtest-input/preregistration-batch2-2026-09-21.md`. Vrai moteur, M1 exact, garde-fous réels rejoués (0,3 %), spread = 2 × le frère (décision). ' + `Spreads absolus : ${Object.entries(SPREAD).map(([k, v]) => `${k} ${v.toPrecision(3)}`).join(', ')}.`, ''];
...
if (MODE === 'train') {
  ...
  for (const h of HYP) {
    ...
    const ok = taken.length >= 80 && per(taken) >= 0.15 && sum(a) > 0 && sum(b) > 0;
  ...
  md.push('', `Retenues pour la lecture du test : ${retained.length ? retained.join(', ') : 'aucune'}.`, '', 'Règle appliquée : ≥ 80 trades, R net par trade ≥ +0,15 à 2 × le spread du frère, et R net positif sur les DEUX moitiés de l\'entraînement (repli prévu quand il y a moins de 3 années civiles complètes). Aucune hypothèse rejetée ici n\'a vu son test.');
  fs.writeFileSync(RETAINED_FILE, JSON.stringify({ decidedAt: new Date().toISOString(), retained }, null, 2));
  fs.writeFileSync('data/backtest-input/prereg-batch2-train.md', md.join('\n'));
  console.log(md.join('\n'));
} else {
  ...
  md.push('| Hypothèse | Trades | R net / trade (2 ×) | R net total | t | Corrélation mensuelle avec le combo | Verdict |', '|---|---|---|---|---|---|---|');
  for (const id of retained) {
    ...
    const mean = per(taken), sd = taken.length > 1 ? Math.sqrt(taken.reduce((a, t) => a + (t.r - mean) ** 2, 0) / (taken.length - 1)) : 0; const tstat = sd > 0 ? mean / (sd / Math.sqrt(taken.length)) : 0;
    const pass = taken.length >= 40 && mean >= 0.12 && sum(taken) > 0 && c < 0.3 && tstat >= 2.0;
    const modest = false;
    md.push(`| ${h.label} | ${taken.length} | ${fmt(per(taken), 3)} | ${fmt(sum(taken), 1)} | ${tstat.toFixed(2)} | ${c.toFixed(2)} | ${pass ? (modest ? '—' : '**RÉUSSIE : candidate (mode alerte)**') : (tstat < 2.0 && mean >= 0.12 && taken.length >= 40 ? 'non concluante (t < 2)' : 'ÉCHEC')} |`);
    if (false) { const t10 = replay(list, sp2, 10, CUT, Infinity); md.push(`| ↳ sensibilité XTIUSD, spread ×10 (sans effet sur la décision) | ${t10.length} | ${fmt(per(t10), 3)} | ${fmt(sum(t10), 1)} | — | — |`); }
  }
  ...
  fs.writeFileSync('data/backtest-input/prereg-batch2-test.md', md.join('\n'));
  console.log(md.join('\n'));
}
