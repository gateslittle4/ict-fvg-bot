#!/usr/bin/env node
// runMentFundingAllLiveStrategiesCycleAccountImpact.js
// Usage: node scripts/runMentFundingAllLiveStrategiesCycleAccountImpact.js <dir-with-csvs>
//
// Third fork of runFtmoAllLiveStrategiesCycleAccountImpact.js (same
// continuous-reset architecture, see that file's header for the full
// rationale), after the CTI one. Esdras (2026-09-12): "Autre platforme
// sérieuse?" after FundedNext and Alpha Capital Group turned out to
// forbid this bot's fully-autonomous trading outright, and The5ers turned
// out 4-5x more expensive than FTMO/CTI. Ment Funding passes BOTH filters:
// EAs/cBots explicitly permitted with no "assist-only" restriction
// ("EAs, hedging, scalping, any strategy - all permitted", sourced live
// from mentfunding.com, 2026-09-12), cTrader supported (already coded in
// this bot, via their broker ThinkMarkets), and a $25k 1-Step evaluation
// price of $250 - in FTMO's range, not CTI's, but not The5ers' either.
// Smaller firm than FTMO (4.9/5 Trustpilot but only ~227 reviews, vs
// FTMO's 8000+ at 4.6 - strong but thin sample, worth noting as a real
// caveat, not just a number).
//
// ONE rule change from the FTMO/CTI scripts that needed actual code, not
// just constants: Ment Funding's 6% max drawdown is STATIC (fixed 6%
// below the STARTING balance, confirmed via the same live fetch - "fixed
// below starting balance, doesn't trail with profits"), not trailing on
// the highest balance/equity reached like FTMO (10%, EOD) or CTI (5%,
// balance-based trailing). A static floor never rises, so once a cycle is
// meaningfully ahead the 6% floor becomes practically irrelevant - very
// different risk shape from a trailing floor that keeps chasing the
// account up. simulateContinuous() below computes ddPct against the fixed
// starting balance instead of against cyclePeak for the bust check; the
// PASS check (+10%) is unchanged. Target: **+10%** (same as FTMO, not
// CTI's 8%). Daily loss 5% of previous day's balance (looser than FTMO's
// 3%, tighter than CTI's none) - not modeled as a separate cap here
// either, same reasoning as the FTMO/CTI scripts (the bot's own
// GuardrailEngine daily protection already applies regardless of firm).
//
// Same 6 scenarios as the CTI report (0.5/0.4/0.3/0.2% risk, 2
// concurrency-cap variants) for direct row-for-row comparison across all
// three reports. Same combined strategy scope, same source, same priority
// order. All non-risk params read straight from CONFIG - no src/ change,
// research only.
...
const STATIC_MAX_LOSS_PCT = 6; // Ment Funding: fixed 6% below STARTING balance, never trails up
const CHALLENGE_TARGET_MULTIPLE = 1.10; // Ment Funding: 10% profit target (same as FTMO)
...
// Same 6 scenarios as the CTI report (baseline, 3 lower-risk variants
// including 0.2%, 2 concurrency-cap variants) for direct comparison across
// all three reports (FTMO/CTI/Ment Funding).
const SCENARIOS = [
...
function simulateContinuous({ candlesBySymbolFull, divergenceCandidatesFull, nwogCandidatesFull, judasCandidatesFull, riskPct, maxConcurrentPositions }) {
  ...
  const resolveClose = (symbol, netR, exitTime, riskAmount, outcome) => {
    ...
    // STATIC floor (Ment Funding): measured against the fixed STARTING
...
function main() {
  ...
  if (!dir) { console.error('Usage: node scripts/runMentFundingAllLiveStrategiesCycleAccountImpact.js <dir-with-csvs>'); process.exit(1); }
  ...
  const md = [];
  md.push('# Simulation continue avec reset au +10% ou au bust (-6% statique) — Ment Funding 1-Step, toutes stratégies live, compte 10k');
  md.push('');
  md.push(
    "Fork direct de ftmo-1step-all-live-strategies-cycle-account-impact.md et cti-1step-all-live-strategies-cycle-" +
      "account-impact.md (même architecture de reset continu). Esdras (2026-09-12), après que FundedNext et Alpha " +
      "Capital Group se soient révélés incompatibles avec un bot 100% autonome (interdisent l'exécution sans " +
      "supervision humaine) et que The5ers se soit révélé 4-5x plus cher : \"Autre platforme sérieuse?\" **Ment " +
      "Funding** passe les deux filtres : EA/cBots explicitement permis sans restriction (\"EAs, hedging, " +
      "scalping, any strategy - all permitted\", sourcé en direct sur mentfunding.com, 2026-09-12), cTrader " +
      "supporté (déjà câblé dans ce bot), prix $25k confirmé à **$250** (dans la fourchette FTMO, pas celle de " +
      "CTI). Firme plus petite que FTMO (4.9/5 Trustpilot mais ~227 avis seulement, contre 8000+ à 4.6 chez FTMO " +
      "- solide mais échantillon mince, à garder en tête).\n\n" +
      "**Différence de règle qui a demandé un vrai changement de code** (pas juste des constantes) : le drawdown " +
      "max 6% de Ment Funding est **STATIQUE** (fixé 6% sous le solde DE DÉPART, confirmé \"fixed below starting " +
      "balance, doesn't trail with profits\"), pas trailing comme FTMO (10%, fin de journée) ou CTI (5%, sur le " +
      "solde). Un plancher statique ne monte jamais - une fois un cycle significativement en avance, le plancher à " +
      "6% sous le DÉPART devient pratiquement hors de portée. `simulateContinuous()` calcule donc le drawdown " +
      "contre le solde de départ fixe, pas contre un pic mobile. Target **+10%** (comme FTMO, pas les +8% de CTI). " +
      "Perte journalière 5% du solde de la veille (plus souple que les 3% de FTMO, plus stricte que l'absence de " +
      "limite chez CTI) - non modélisée séparément ici, même raisonnement que les scripts FTMO/CTI (la protection " +
      "journalière propre du bot reste active quelle que soit la firme).\n\n" +
  ...
  md.push('');
  md.push(
    `Contre le plancher STATIQUE de Ment Funding (6% sous le départ, jamais trailing), le scénario actuel ` +
      ...
      `plancher 10% trailing) et CTI (34% à 0.5%, plancher 5% trailing) pour mesurer l'effet réel d'un plancher ` +
      `statique plutôt que mobile.\n\n` +
  ...
  const outMd = path.join(dir, 'ment-funding-1step-all-live-strategies-cycle-account-impact.md');
