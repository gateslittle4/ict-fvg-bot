...
# Sortie : data/real-m1-history/skip-<N>/<PAIRE>.csv (N = jours en arriere ; skip-0 et skip-245 existent deja ailleurs, on les saute).
...
empty_in_a_row=0
for SKIP in $(seq 490 245 5000); do
  ...
  for sym in $SYMS; do
    ...
    HDR=$(mktemp)
    curl -s -m 240 -D "$HDR" -o "$DIR/$sym.csv" "$BASE?symbol=$sym&timeframe=M1&days=245&skip=$SKIP&token=$ADMIN_EXPORT_TOKEN"
    ...
    rm -f "$HDR"
    echo "skip=$SKIP $sym : $STATUS | ${COUNT:-0} bougies | debut ${FIRST:--}"
