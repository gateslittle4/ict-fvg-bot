#!/usr/bin/env node
// runFvgDivRsiAccountImpact.js
// Usage: node scripts/runFvgDivRsiAccountImpact.js <dir-with-csvs>
//
// Direct test of "do we need a 3rd instrument, or is stacking more
// independent STRATEGIES on the SAME 2 pairs (US100/US500) enough" - take
// 2 of runThreeStrategyTwoPairsAccountImpact.js, swapping the 3rd
// mechanism from Turtle (which failed: its 30+ day median hold crowded out
// Divergence ~46% of the time under strict netting, see
// three-strategy-two-pairs-account-impact.md) for RSI(2) mean-reversion
// (Larry Connors), which validated solo with a real edge AND a short
// holding period (max 10 days) specifically chosen to be netting-
// compatible - see rsi-mean-reversion-analysis.md.
//   - FVG (validated ICT setup, existing engines).
//   - Divergence (US100/US500 pairs mean-reversion, lookback=100, z=2).
//   - RSI(2) mean-reversion (single-instrument, EMA200 trend filter, 2xATR
//     stop, SMA5 mean-reversion exit, 10-day timeout) - published Connors
//     parameters, not tuned on our data.
//
// THREE-WAY NETTING: at most ONE open position per instrument, across ALL
// THREE mechanisms - whichever source gets there first blocks the other
// two on that same instrument until it closes.
//
// RSI(2) runs on DAILY bars (resampled from M15, UTC day buckets),
...
// attempt from all three sources in true chronological order.
//
// FTMO 1-Step rules (see ftmo-1step-account-impact.md): single +10%
// target, bust = TRAILING off the highest balance ever reached (not
// static off the starting balance). Own guardrail daily-loss cap stays at
// the self-imposed 2% (stricter than FTMO's real 3% allowance).
...
function simulateYear(m15CandlesBySymbolFull, dailyCandlesBySymbolFull, divergenceCandidatesFull, year) {
  ...
  // convention as the FVG/Divergence side of this simulation).
  const rsiEma200 = {};
  const rsiValues = {};
  const rsiSma5 = {};
  const rsiAtr = {};
  ...
  // Merged, time-sorted event timeline: M15 candles (FVG + Divergence) and once-per-day RSI(2)
  ...
  const openRsi = {}; for (const s of FVG_SYMBOLS) openRsi[s] = null;
...
function fmtRow(year, r) {
  ...
  return `| ${year}${TRAIN_YEARS.has(year) ? ' (train)' : ' (test)'} | ${r.trades} (${r.fvgTrades} FVG + ${r.divTrades} div. + ${r.rsiTrades} RSI-2) | ${wr} | ${r.staticDrawdownPct.toFixed(1)}% | ${r.trailingDrawdownPct.toFixed(1)}% | ${bustCell} | ${r.challengeLabel} | $${r.finalBalance.toFixed(0)} |`;
}

function main() {
  const dir = process.argv[2];
  if (!dir) {
    console.error('Usage: node scripts/runFvgDivRsiAccountImpact.js <dir-with-csvs>');
    process.exit(1);
  }
  ...
  const md = [];
  md.push('# Impact au niveau du COMPTE — 3 STRATÉGIES (FVG + Divergence + RSI(2) mean-reversion), seulement US100/US500, règles FTMO 1-Step');
  md.push('');
  md.push(
    `⚠ Compte $${STARTING_BALANCE}, AUCUN 3e instrument (pas d'or ici) - 2e tentative de : empiler des stratégies ` +
      "sur les mêmes 2 paires suffit-il ? Cette fois avec RSI(2) au lieu de Turtle (qui avait échoué - voir " +
      "three-strategy-two-pairs-account-impact.md - sa durée de position trop longue bloquait Divergence ~46% du " +
      "temps sous le netting strict). Un seul budget de garde-fous PARTAGÉ entre les 3 mécanismes, garde-fous " +
      "INCHANGÉS (max 2 trades/jour, cooldown 30min après perte, perte quotidienne max 2%). NETTING À TROIS : au " +
      "plus une position ouverte par instrument, tous mécanismes confondus. RSI(2) = filtre de tendance EMA200, " +
      "entrée RSI(2)<5 (haussier) ou >95 (baissier), stop 2xATR(14), sortie SMA(5) ou 10 jours max - paramètres " +
      "ORIGINAUX publiés (Connors, pas ajustés sur nos données - voir rsi-mean-reversion-analysis.md). **Cible = " +
      "+10% une seule fois (FTMO 1-Step). Perte totale max = TRAILING sur le plus haut solde jamais atteint (10%).**"
  );
  md.push('');
  ...
  for (const year of YEARS) {
    ...
    console.error(`${year}: ${r ? r.trades + ' trades (' + r.fvgTrades + ' FVG + ' + r.divTrades + ' div + ' + r.rsiTrades + ' RSI-2), solde $' + r.finalBalance.toFixed(0) + ', challenge: ' + r.challengeLabel : 'skip'}`);
  }

  const outMd = path.join(dir, 'fvg-div-rsi-account-impact.md');
