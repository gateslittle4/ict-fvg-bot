#!/usr/bin/env node
// runWeeklySweepStrategyAnalysis.js
// Usage: node scripts/runWeeklySweepStrategyAnalysis.js <dir-with-csvs>
//
// ICT "Weekly Liquidity Sweep" (PWH/PWL sweep+reclaim) - already LIVE on
// GER40 only (see config.js's weeklySweep block). Never tested with the same
// 8-symbol/train-test rigor as NWOG/Judas Swing/Breaker Block - this fills
// that gap. Same conventions as those scripts: fixed 1:3 R:R, 480 M15-candle
// timeout (src/backtest/weeklyLiquiditySweep.js's own defaults), screened on
// TRAIN (2019-2023), verified on TEST (2024-2025), same verdict rule.
...
function main() {
  ...
  if (!dir) {
    console.error('Usage: node scripts/runWeeklySweepStrategyAnalysis.js <dir-with-csvs>');
    process.exit(1);
  }

  const md = [];
  md.push('# Weekly Liquidity Sweep (PWH/PWL sweep+reclaim) sur les 8 instruments - déjà en production sur GER40 seul');
  md.push('');
  md.push(
    "⚠ Déjà LIVE sur GER40 (voir config.js `weeklySweep`), jamais testé avec la même rigueur train/test sur les " +
      '7 autres instruments. Même code exact (`runWeeklySweepBacktest`), cible fixe 1:3, timeout 480 bougies M15. ' +
  ...
  const outMd = path.join(dir, 'weekly-sweep-strategy-analysis.md');
