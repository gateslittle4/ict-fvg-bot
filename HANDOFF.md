...
## Forward-test réel de CBDR (avant tout ajout au live) — 2026-09-18

Esdras, après avoir vu CBDR classé "candidat sérieux" (section précédente) et proposé deux options (l'ajouter au combo existant, ou le tester séparément sur un autre compte/challenge) : **"On le teste sérieusement avant. Comme les autres [paires]."** — même rigueur que Silver Bullet avant sa mise en live (train/test historique + un vrai forward-test sur données broker jamais touchées), pas un passage direct en production.

**Nouveau script `scripts/runCbdrForwardTestAnalysis.js`**, calqué directement sur `runSilverBulletForwardTestAnalysis.js` (même conversion de fuseau horaire UTC réel → convention moteur, même mise en garde sur la taille de fenêtre, mêmes paramètres publiés/jamais retouchés). Exécuté sur `data/real-data-2026-02-to-09/` — les vraies bougies cTrader de production (2026-02-10 → 2026-09-16) déjà exportées et committées pour le forward-test de Silver Bullet, jamais utilisées pour choisir un paramètre de CBDR.

**Écart de portée assumé, pas contourné en silence** : CBDR tenait sur 4 instruments en historique (US100, EURUSD, GBPUSD, GER40) mais **aucun export réel de GBPUSD n'existe dans ce dépôt** — testé sur 3 des 4 (US100, EURUSD, GER40), GBPUSD reste non vérifié sur données réelles plutôt que deviné.

**Résultat, `data/real-data-2026-02-to-09/cbdr-forward-test.md`** :
- **US100** (52 trades) : espérance **+0.34R** — tient, même plus fort qu'en historique (test 0.18R).
- **EURUSD** (31 trades) : espérance **+0.73R** — tient, nettement plus fort qu'en historique (test 0.13R), échantillon plus modeste.
- **GER40** (74 trades) : espérance **-0.07R** — **ne tient PAS**, alors que c'était le résultat historique le plus fort de tous (1447/279 trades, test 0.21R). Un vrai signal du forward-test qui contredit l'historique — exactement le genre de chose que ce test existe pour attraper.

**Conclusion** : résultat mitigé, pas un feu vert global. 2/3 instruments testés confirment le signal historique sur données réelles jamais vues, 1/3 (justement le plus fort en historique) ne le confirme pas. Ni recommandé ni rejeté pour du capital réel à partir de ce seul résultat — à décider avec Esdras, probablement en excluant GER40 du candidat si CBDR avance, ou en creusant pourquoi GER40 échoue spécifiquement sur cette fenêtre avant de trancher.

**Précision en passant (répondant à la question d'Esdras "tu l'as déjà testé dans les 15 années passées?")** : vérifié que `data/backtest-input/US100.csv` et `GER40.csv` remontent en réalité à 2010 (~15 ans de données brutes disponibles), pas seulement 2019 comme la documentation du projet le répète par habitude ("TRAIN 2019-2023") — le filtre du script (`c.time < TRAIN_CUTOFF`, sans borne basse) a donc déjà silencieusement englobé toute cette profondeur pour US100/GER40 (et XAUUSD/USDCAD, mêmes dates de départ ~2009-2010) dans le découpage historique existant. EURUSD/GBPUSD/UKX/AUX, eux, démarrent réellement en 2018-2019 — l'étiquette "2019-2025" leur correspond, pas aux autres. Pas un bug de calcul (la coupure train/test au 2024-01-01 reste correcte) — juste une description imprécise à corriger si quelqu'un s'appuie dessus pour estimer la profondeur réelle des tests. Pas corrigé dans ce fichier (cosmétique, hors sujet du forward-test demandé), signalé pour référence future.

**Fichiers** : `scripts/runCbdrForwardTestAnalysis.js` (nouveau), `data/real-data-2026-02-to-09/cbdr-forward-test.md` (nouveau). Aucun changement de comportement live — CBDR reste au stade recherche, comme avant. `npm test` : 710/710 (inchangé, script d'analyse seulement).

## Correction de méthode : le TRAIN historique de CBDR utilisait déjà les 15 ans, l'étiquette était fausse — 2026-09-18 (suite)

Esdras, après le forward-test réel : **"Ce n'est pas la méthode. On étudie d'abord les données passées, donc tu fais le test sur les 15 ans."** — l'ordre correct est d'abord tout l'historique disponible, PUIS le forward-test réel en confirmation, pas l'inverse.

**Vérifié en premier plutôt que supposé** : `scripts/runCbdrStrategyAnalysis.js`'s filtre TRAIN (`c.time < TRAIN_CUTOFF`, cutoff 2024-01-01) n'a **aucune borne basse** — il englobait déjà TOUT ce que chaque CSV contient avant 2024, quelle que soit sa profondeur. Or `data/backtest-input/US100.csv`/`GER40.csv` démarrent en réalité le **2010-11-14/15** (XAUUSD 2009-03-15, USDCAD 2010-01-03) — pas 2019 comme l'étiquette "TRAIN (2019-2023)" du script (et de la section CBDR précédente de ce fichier) le laissait croire par habitude. **Le calcul utilisait donc déjà silencieusement ~13-15 ans de TRAIN pour ces symboles-là**, tout du long — pas un bug de calcul (la coupure au 2024-01-01 reste correcte), juste une étiquette fausse qui sous-estimait la vraie profondeur testée. EURUSD/GBPUSD/UKX/AUX, eux, démarrent réellement en 2018-2019 — pour ces quatre-là, "2019-2023" était exact.

**Corrigé** : `scripts/runCbdrStrategyAnalysis.js` affiche maintenant une colonne "Début train" avec la vraie date de départ lue directement dans le CSV de chaque symbole, au lieu d'une étiquette générique. Rapport régénéré (`data/backtest-input/cbdr-strategy-analysis.md`) — **les chiffres n'ont PAS changé** (US100 toujours 694 trades train/166 test, GER40 toujours 1447/279, etc.), confirmant que le calcul était déjà correct, seule la description était trompeuse. Verdicts inchangés : US100/EURUSD/GBPUSD/GER40 tiennent, US500/XAUUSD/USDJPY affaiblis, USDCAD/UKX/AUX rejetés.
...
## Pourquoi GER40 échoue sur le forward-test réel de CBDR (creusé sur demande d'Esdras) — 2026-09-18 (suite)

Esdras : "On creuse pourquoi GER40 échoue sur le forward-test réel." Analyse ad-hoc (script jetable, pas committé — les résultats suffisent) sur les 74 trades GER40 du forward-test (`data/real-data-2026-02-to-09/GER40.csv`), ventilés par mois.

**Un seul mois explique tout** : août 2026 (12 trades, **12 pertes, 0 gain, -12.59R**) à lui seul fait basculer le résultat global en négatif. Les 7 autres mois combinés (62 trades) donnent une espérance de **+0.12R** (WR 29%, PF 1.17) — quasiment identique au TRAIN historique (+0.10R) et au TEST historique (+0.21R). Le signal se comporte normalement 7 mois sur 8 ; ce n'est pas une dégradation généralisée du mécanisme.

**Mécanisme identifié** : en août, le DAX a fait deux jambes de hausse rapide (25865→26447 début août, 26019→26630 fin août) séparées par un repli — un régime tendanciel-mais-choppy, le pire cas de figure pour une stratégie de fade (retournement). Chaque jambe a poussé le prix au-delà de la projection 2x haussière de CBDR, déclenchant des fades baissiers systématiquement stoppés puisque la tendance continuait au lieu de retourner (9 trades baissiers, 0 gain, -9.45R). Même les 3 fades haussiers sur les creux intermédiaires ont échoué.

**Comparé à l'historique complet plutôt que traité comme une anomalie isolée** : sur les 182 mois de `GER40.csv` (2010-2025), la moyenne mensuelle est +1.15R avec un écart-type de ±5.1R, et il existe déjà 2 mois historiques à 0% de réussite avec échantillon comparable (juillet 2021 et août 2023, tous deux -9.6R/8 trades). Août 2026 (-12.59R/12 trades) est plus sévère que le pire mois historique (-9.70R en 2014-11) mais du même type d'événement — une queue de distribution rare, pas un phénomène jamais vu.
...
## EN COURS — analyse d'overlap CBDR/US100 vs production, interrompue avant la fin — 2026-09-18 (suite)

Esdras a demandé de creuser si US100 pouvait être ajouté comme "combo" CBDR (voir sections précédentes : US100 est le seul candidat CBDR qui passe les 3 filtres — profondeur, train/test, forward-test réel — sans réserve). Question posée : les mécanismes peuvent-ils se déclencher simultanément sur US100 ?

**Vérifié et confirmé (pas juste supposé)** : `src/liveStrategyEngine.js:536` — `if (this.openPositions.has(symbol)) return 'netting';`. `openPositions` est une Map PAR SYMBOLE, partagée entre TOUS les mécanismes (FVG, Divergence, NWOG, Silver Bullet aujourd'hui sur US100). Une seule position ouverte à la fois par symbole, premier signal arrivé bloque les autres jusqu'à la clôture. Donc ajouter CBDR ne empilerait PAS de risque supplémentaire — il compétitionnerait pour le même créneau que les 4 mécanismes déjà live sur US100. Conséquence : les chiffres CBDR déjà présentés (train/test/forward-test) sont calculés EN AUTONOME, sans tenir compte de cette compétition — ils surestiment probablement ce que CBDR apporterait réellement une fois en concurrence avec FVG/Divergence/NWOG/Silver Bullet.

**Objectif de l'analyse demandée** : mesurer, pour chaque trade CBDR standalone sur US100 (`runCbdrBacktest()`, `src/backtest/cbdr.js`), si sa fenêtre [entryTime, exitTime] chevauche celle d'un trade déjà pris par un mécanisme EN PRODUCTION sur US100 — même méthode que `scripts/runSilverBulletOverlapAnalysis.js` / `data/backtest-input/silver-bullet-overlap-analysis.md` (déjà fait pour Silver Bullet avant sa mise en live, sur US100/US500/GER40).

**Où j'en étais, précisément, avant l'interruption** :
- Le template existant (`runSilverBulletOverlapAnalysis.js`) reconstruit la "production" en appelant CHAQUE mécanisme en autonome (ex. `runBacktest()` pour FVG, `runNwogBacktest()` pour NWOG) et en les concaténant — SANS simuler le netting RÉEL entre eux. Et il ne couvre PAS Divergence (pas de module de backtest autonome propre pour Divergence — elle n'existe qu'intégrée dans `liveStrategyEngine.js` (`_computeDivergenceCandidates`/`_processDivergenceCandidate`) et dans la simulation bespoke de `scripts/runFtmo1StepAccountImpact.js`). Sur US100 aujourd'hui, Divergence EST live (paire US100/US500) — l'ignorer serait incomplet.
- Piste que j'explorais, plus fidèle que le template : utiliser directement `LiveStrategyEngine.warmUp(candlesBySymbol, { onEvent })` (`src/liveStrategyEngine.js:1323-1471`) pour rejouer la VRAIE logique de production (FVG + Divergence + NWOG + Silver Bullet, exactement la config de `config.js`) sur l'historique complet US100+US500, et capturer via `onEvent(signal, candle)` :
  - une OUVERTURE : `signal.type === 'validated' && !signal.blockedReason` (uniforme sur tous les mécanismes, vérifié dans le code — `entryTime` à prendre sur `candle.time` passé en 2e argument du callback, PAS un champ du signal lui-même) ;
  - une CLÔTURE : événement `{ type: 'closed', source, symbol, id, direction, outcome, entryPrice, stopPrice, targetPrice, exitTime }` (pas d'`entryTime` dessus non plus — à apparier avec l'ouverture précédente sur le même symbole via une map `pendingOpen[symbol]`, fiable car le netting garantit au plus une position ouverte à la fois par symbole).
  - Ça donnerait la VRAIE liste de trades production (netting réel inclus, Divergence incluse) au lieu de l'approximation du template Silver Bullet — plus rigoureux.
- **Point bloquant où je me suis arrêté** : `LiveStrategyEngine` exige un `guardrail` (instance `GuardrailEngine`) au constructeur — `_blockReason()` l'appelle (`this.guardrail.canTakeNewTrade(guardrailNow, symbol)`, `src/liveStrategyEngine.js:543`). Décision à prendre avant de continuer : passer un guardrail permissif/factice (puisque les limites de guardrail sont des contraintes de compte réel, hors sujet pour la question "est-ce que deux SIGNAUX se chevauchent dans le temps" — même esprit que le template Silver Bullet qui ignore aussi les guardrails), ou reconstruire un `GuardrailEngine` réel avec des limites larges. Pas encore tranché ni codé.

**Reste à faire pour boucler cette analyse** :
1. Décider et coder le guardrail factice/permissif pour l'instanciation de `LiveStrategyEngine`.
2. Construire la config de production réelle pour US100 à partir de `config.js` (copier, pas re-dériver — même précaution que `FVG_PROD_CONFIG` dans le template Silver Bullet, avec la mention explicite que ça peut devenir obsolète si `config.js` change) : `fvg.perSymbol.US100` (multiTouch, H4_EMA200, rr 5, fenêtre 8h-12h), `divergence` (pair US100/US500, lookback 100, zThreshold 2, etc. — valeurs exactes à relire dans `config.js:257+`), `nwog` (symbols US100/GER40, longOnlySymbols US100, rr 5), `silverBullet` (symbols US100/US500/GER40, rr 3).
3. Lancer `warmUp()` sur US100+US500 (les deux nécessaires pour la paire Divergence), collecter tous les trades production tagués par mécanisme.
4. Charger les trades CBDR standalone sur US100 (`runCbdrBacktest()`) et calculer le chevauchement (même sens = double exposition, sens opposé = contradiction, aucun chevauchement = vraiment additif) — même tableau de sortie que `silver-bullet-overlap-analysis.md`.
5. Écrire le script sous `scripts/runCbdrOverlapAnalysis.js`, le rapport sous `data/backtest-input/cbdr-us100-overlap-analysis.md`, documenter le résultat ici dans HANDOFF, `npm test` avant de commit/push.

**Contexte pour la suite** : CBDR reste au stade recherche, rien n'a été ajouté au live. Cette analyse d'overlap est la dernière étape avant qu'Esdras puisse prendre une décision informée sur l'ajout d'US100/CBDR comme mécanisme live. Prochaine session Claude : reprendre directement à l'étape 1 ci-dessus, tout le contexte nécessaire (fichiers, lignes, décisions déjà prises) est dans cette section.

## Analyse d'overlap CBDR/US100 vs production — terminée — 2026-09-18 (suite)

Suite directe de la section précédente, reprise dans la même session ("Continue ton analyse"). Terminé les 5 étapes listées : guardrail factice (`{ canTakeNewTrade: () => true }` — les limites de compte réel sont hors sujet pour une question de chevauchement temporel de signaux, même omission que le template Silver Bullet), config de production copiée verbatim de `config.js` (fvg.perSymbol.US100, divergence, nwog, silverBullet), `LiveStrategyEngine.warmUp()` lancé sur US100+US500, comparaison avec les trades CBDR standalone (`runCbdrBacktest()`).

**Bug trouvé et corrigé en cours de route** : le premier essai donnait **zéro trade Divergence** dans la reconstruction de production, alors qu'elle est live sur US100. Cause : `warmUp()` traite les symboles dans l'ordre des clés de l'objet passé, et `_warmUpOneSymbol` a un comportement documenté (pré-existant, pas introduit ici) où le PREMIER symbole d'une paire Divergence traité voit l'historique de son partenaire encore vide à ce moment-là, et ne découvre donc JAMAIS ses propres candidats Divergence pour le reste du replay. En passant `{ US100, US500 }` (US100 en premier), US100 perdait silencieusement 100% de ses trades Divergence. Corrigé en inversant l'ordre (`{ US500, US100 }`) — US500 fini d'être traité avant que le tour d'US100 n'arrive, donc l'historique du partenaire est déjà complet. Après correction : 716 trades Divergence apparaissent (au lieu de 0), et les comptes des 3 autres mécanismes changent aussi légèrement (silverbullet 1269→1124, nwog 294→265, fvg 1942→1736) — logique : avec Divergence qui prend maintenant correctement sa part du créneau netté, elle bloque certains signaux qui, avant la correction, avaient le champ libre à tort.

**Résultat** (`data/backtest-input/cbdr-us100-overlap-analysis.md`) : sur US100, la reconstruction réelle (netting inclus, chemin de code identique au bot live) donne **3841 trades de production sur l'historique complet** — Silver Bullet 1124, FVG 1736, Divergence 716, NWOG 265. Des 1245 trades CBDR standalone :
- **884 (71.0%) ne chevauchent AUCUN trade de production** — vraiment additif, pas de compétition pour le créneau.
- **361 (29.0%) chevauchent au moins un trade déjà en cours** : 143 (11.5%) même sens (double exposition au même mouvement), 228 (18.3%) sens opposé (deux mécanismes qui se contrediraient au même moment).
- Par mécanisme concurrent : Divergence chevauche le plus (152 trades CBDR), puis Silver Bullet (121), FVG (111), NWOG (17, cohérent avec son rythme hebdomadaire).

**Lecture, sans trancher pour Esdras** : 71% sans chevauchement est du même ordre de grandeur que le chiffre qui avait justifié le déploiement de Silver Bullet (76-83% sans chevauchement, voir `silver-bullet-overlap-analysis.md`) — CBDR sur US100 apporterait donc majoritairement de l'exposition NEUVE, pas une redite de ce qui tourne déjà. Mais ce chiffre est calculé sur le NOMBRE de trades qui se chevauchent, pas sur leur R — un chevauchement fréquent avec Divergence en particulier (152 trades) mériterait d'être regardé d'un peu plus près si CBDR avance vers le live, pour vérifier que ce n'est pas concentré sur les trades les plus rentables de l'un ou l'autre mécanisme.

**Reste ouvert, pas fait ici** (à la demande d'Esdras, qui est passé à un autre sujet — voir section suivante) : porter CBDR en mécanisme live réel dans `liveStrategyEngine.js` (rappel : CBDR n'existe qu'en backtest aujourd'hui, `src/backtest/cbdr.js` — voir la section "Verdict mis à jour sur US100/CBDR" plus haut pour le détail de ce que ça demanderait). Décision de mise en live non prise — recherche uniquement.

**Fichiers** : `scripts/runCbdrOverlapAnalysis.js` (nouveau), `data/backtest-input/cbdr-us100-overlap-analysis.md` (nouveau). Aucun changement de code live ni de comportement du bot — script d'analyse autonome uniquement. `npm test` : 710/710.

## CBDR câblé en mécanisme live réel, US100 uniquement — 2026-09-18 (suite)

Esdras : **"Cable alors cbdr"**, après une vérification complète (demandée séparément, faite dans le chat juste avant celle-ci, pas documentée en section séparée ici) du pipeline signal→ordre réel : confirmé en lisant le code que `cTraderDataSource.js` envoie un vrai `ProtoOANewOrderReq` (`_submitOrder`, ligne ~1863) dès qu'un signal `validated`/`blockedReason: null` arrive ET que `isAutoExecuteActive()` est vrai, avec confirmation broker réelle (`ProtoOAExecutionEvent`) et un fallback de réconciliation (`ProtoOAReconcileReq`) si la confirmation n'arrive pas — chemin testé (13 tests dédiés, dont un qui reproduit l'incident réel du 2026-09-17). CBDR n'existait qu'en backtest (`src/backtest/cbdr.js`) jusqu'ici — jamais câblé dans `liveStrategyEngine.js`. Porté maintenant, sur le même modèle que NWOG/Silver Bullet (entrée à `candle.open`, pas de zone FVG) — méthode la plus proche déjà en production.

**Changements** (mêmes 5 points que la checklist laissée dans la section "Verdict mis à jour sur US100/CBDR" plus haut) :
- `src/liveStrategyEngine.js` : `cbdrConfig` ajouté au constructeur (même pattern opt-in que `nwogConfig`/`silverBulletConfig` — jamais par défaut, pour ne pas affecter silencieusement les 3 moteurs de backtest/rapport historiques). `_computeCbdrCandidates()`/`_detectCbdrSignal()`/`_processCbdrCandidate()` ajoutées, copiant exactement le patron de `_processNwogCandidate()` (entrée = `candle.open`, stop = `sweepExtreme`, cible = RR fixe, garde `validStopSide`). Câblé dans `ingestCandle()` ET dans `warmUp()`/`_warmUpOneSymbol()` (précalcul + boucle par bougie). Participe au MÊME `openPositions`/netting que FVG/Divergence/NWOG/Silver Bullet — pas de suivi de position séparé. Pas de filtre de direction (CBDR est bidirectionnel, jamais séparé long/short contrairement à NWOG/US100).
- `src/config.js` : nouveau bloc `cbdr: { symbols: ['US100'], rrMultiple: 3, maxHoldingM15Candles: 480 }` — valeurs exactes déjà validées (RR_MULTIPLE/MAX_HOLDING_CANDLES par défaut de `cbdr.js`), pas re-tunées. Commenté en détail avec le résumé des 3 couches de validation (profondeur 15 ans, forward-test réel +0.34R, overlap 71% non-chevauchant) et la justification explicite de l'exclusion de GER40/EURUSD/GBPUSD.
- `src/accountRegistry.js` : `cbdr: CONFIG.cbdr` ajouté à `buildEffectiveConfig()` — **attention particulière ici** : c'est exactement la classe de bug trouvée le 2026-09-17 pour weeklySweep/breakerBlock (oubliés dans ce fichier, silencieusement inertes en production pendant ~2 jours malgré la doc "LIVE"). Évité cette fois en l'ajoutant dans le MÊME changement que le bloc `config.js`.
- `src/accountRuntime.js` : `cbdrConfig: config.cbdr` ajouté à l'instanciation réelle de `LiveStrategyEngine` (le seul moteur qui trade réellement).
- `test/accountRegistry.test.js` : le test de régression existant (qui garde justement weeklySweep/breakerBlock contre ce même oubli) étendu avec `cbdr` dès sa création, plutôt qu'après coup.
- `test/liveStrategyEngine.test.js` : 4 nouveaux tests (signal→position réelle avec entry/stop/target corrects, résolution WIN, résolution LOSS, netting bloque/est bloqué par les autres mécanismes) — fixture reprise telle quelle de `test/cbdr.test.js` (module de backtest pur, déjà existant).

**`npm test` : 714/714** (710 + 4 nouveaux).

**Portée délibérément limitée à US100 seulement** — c'est le seul symbole qui a passé les 3 couches de validation sans réserve cette session (voir sections précédentes). GER40 (échec du forward-test réel malgré le meilleur historique), EURUSD et GBPUSD (affaiblis une fois la profondeur complète prise en compte) restent HORS de `config.js`'s `cbdr.symbols` — pas ajoutés.

**Ce qui N'A PAS été fait, à trancher avec Esdras avant tout déploiement réel** :
...
## Simulation compte complet : les 8 mécanismes live combinés contre le cycle FTMO 1-Step (10%) — 2026-09-18 (suite)

Esdras : "Vérifie pour moi tous les strategy combiné, le cycle de 10% de FTMO avec tous ses contraintes pour les 7 derniers mois et cette semaine." Nouveau script `scripts/runFtmo1StepFullComboAccountImpact.js`, une approche différente de tous les précédents scripts d'impact-compte du projet : au lieu de réimplémenter une simulation de netting maison par mécanisme, il rejoue la VRAIE logique de production de bout en bout — `LiveStrategyEngine` construit directement depuis `CONFIG` (les 8 mécanismes actuellement live : FVG, Divergence, NWOG, Judas Swing, Weekly Sweep, Breaker Block, Silver Bullet, CBDR), et un vrai `GuardrailEngine` configuré via `accountRegistry.buildEffectiveConfig({ propFirmProgramId: 'ftmo-1step' })` — pas des valeurs recopiées à la main, donc jamais de dérive silencieuse avec `ftmo.js`/`config.js`.

**Données** : fusion dédupliquée par timestamp de `data/real-data-2026-02-to-09/` (2026-02-13 → 2026-09-16) et `data/real-data-2026-09-17/` (2026-07-02 → 2026-09-17, l'export le plus récent) — fenêtre réelle continue 2026-02-10 → 2026-09-17, aussi proche d'"aujourd'hui" que les données déjà exportées dans ce dépôt le permettent.

**Bug trouvé et corrigé en cours de route** : le même bug d'ordre de warm-up que pour `runCbdrOverlapAnalysis.js` (US500 doit être traité avant US100 pour que Divergence découvre ses propres candidats), mais cette fois-ci, malgré avoir construit `candlesBySymbol` dans le bon ordre, le paramètre `symbols` passé au constructeur de `LiveStrategyEngine` (qui contrôle réellement l'ordre de `warmUp()`, PAS l'ordre des clés de l'objet `candlesBySymbol`) était resté sur l'ordre non trié de `CONFIG.symbols` (US100 en premier). Corrigé en passant le même tableau ordonné aux deux endroits. Effet mesuré : 41 → 71 trades sur le passage principal, date d'atteinte de la cible +10% décalée du 16 avril au 6 juillet — pas un détail cosmétique, une vraie différence de résultat.

**Deux passages distincts, mêmes données** :
- **"réel"** (cible FTMO active, `targetPct: 10`) : répond à "est-ce que le challenge passe" — une fois +10% atteint, la règle réelle du bot bloque tout nouveau trade (`profit_target_reached`, comportement en production depuis a549095), donc peu de trades après cette date.
- **"activité continue"** (`targetPct: null`, tout le reste identique — perte quotidienne 3%, drawdown 10%, max 3 trades/jour, cooldown 30 min) : sert UNIQUEMENT à répondre à "cette semaine" sans que la réponse soit artificiellement vide juste parce que le compte réel se serait arrêté des mois plus tôt après avoir gagné. Pas un vrai compte FTMO — personne ne continue de trader un challenge déjà réussi.

**Résultat, `data/real-data-2026-02-to-09/ftmo-1step-full-combo-account-impact.md`** :
- **7 mois (passage réel)** : 71 trades (20 gagnants, 51 perdants, WR 28.2%), solde final +11.67%. **Cible +10% atteinte le 2026-07-06.** Drawdown max de 10% jamais franchi. Perte quotidienne max de 3% jamais atteinte. Répartition : Silver Bullet (36 trades, +$311.77), Weekly Sweep (18 trades, -$34.15), FVG (17 trades, +$888.89) — les autres mécanismes (Divergence, NWOG, Judas Swing, Breaker Block, CBDR) n'ont simplement gagné aucun créneau netté avant que la cible soit atteinte sur cette fenêtre précise, pas une anomalie : cohérent avec l'analyse d'overlap déjà faite pour CBDR (29% de compétition pour le même créneau).
- **Cette semaine (2026-09-10 → 2026-09-17, passage activité continue)** : 12 trades (6 gagnants), PnL +$1008.84 sur ce passage. Détail complet par trade dans le rapport (US500/US100/XAUUSD, mélange FVG/Weekly Sweep/Divergence/CBDR/NWOG/Silver Bullet).

**Limite assumée** : le pyramidage (`CONFIG.pyramid`) est exclu — désactivé par défaut, non reproduit ici. Si actif en réel, ce résultat sous-estime légèrement le volume de trades.

**Fichiers** : `scripts/runFtmo1StepFullComboAccountImpact.js` (nouveau), `data/real-data-2026-02-to-09/ftmo-1step-full-combo-account-impact.md` (nouveau). Aucun changement de code live — script d'analyse autonome uniquement, aucune donnée du dépôt modifiée à part le nouveau rapport.

## Merge du câblage CBDR live (branche `claude/nouvelle-session-p1lxw4`) — 2026-09-18 (suite)

Esdras a confirmé le déploiement live de CBDR/US100 après avoir vu le contexte complet : mécanisme déjà câblé et testé sur `claude/nouvelle-session-p1lxw4` (714/714).

Entre-temps cette session avait elle-même avancé sur `claude/lire-handoff-hxisa5` (le forward-test CBDR/VWAP ancré ci-dessus), donc un simple fast-forward n'était plus possible — un vrai merge était nécessaire. **Fait ici** : `git merge origin/claude/nouvelle-session-p1lxw4`, 2 conflits résolus (`HANDOFF.md` — les deux sessions avaient ajouté du contenu en fin de fichier, les deux gardés ; `data/real-data-2026-02-to-09/cbdr-forward-test.md` — les deux sessions avaient produit indépendamment un forward-test CBDR sur les mêmes données réelles, chiffres identiques sur les 3 symboles en commun US100/GER40/EURUSD — bonne validation croisée, fusionnés en un seul tableau à 5 symboles). `npm test` : 714/714 après résolution.

**Résolution finalement retenue** : plutôt que de laisser cette branche dans un état intermédiaire non commité, ce merge a été scindé en deux. Tout ce qui est recherche/analyse/documentation (`HANDOFF.md`, `data/backtest-input/cbdr-strategy-analysis.md` mis à jour, `data/backtest-input/cbdr-us100-overlap-analysis.md`, `data/real-data-2026-02-to-09/cbdr-forward-test.md` fusionné, `data/real-data-2026-02-to-09/ftmo-1step-full-combo-account-impact.md`, `scripts/runCbdrForwardTestAnalysis.js`, `scripts/runCbdrOverlapAnalysis.js`, `scripts/runFtmo1StepFullComboAccountImpact.js`, `scripts/runCbdrStrategyAnalysis.js` mis à jour, `data/backtest-input/EURUSD.csv`/`GBPUSD.csv` étendus à 2010) est commité ici. **Le câblage live lui-même (`src/liveStrategyEngine.js`, `src/config.js`, `src/accountRuntime.js`, `src/accountRegistry.js`, `test/accountRegistry.test.js`, `test/liveStrategyEngine.test.js`) reste volontairement HORS de cette branche** — ces 6 fichiers sont revenus à leur version d'avant ce merge (`git checkout HEAD -- <fichiers>`), donc CBDR n'est PAS actif en production sur `claude/lire-handoff-hxisa5` à ce stade. `npm test` : 710/710 (sans les 4 tests du câblage live, qui restent sur `claude/nouvelle-session-p1lxw4`).

**Pour finaliser le câblage live** (nécessite une action directe d'Esdras, ou une règle de permission autorisant ce type d'action à une session Claude Code) : `git fetch origin claude/nouvelle-session-p1lxw4 && git merge origin/claude/nouvelle-session-p1lxw4` sur `claude/lire-handoff-hxisa5` — cette fois sans conflit attendu sur `HANDOFF.md`/`cbdr-forward-test.md` (déjà résolus ici), seuls les 6 fichiers ci-dessus apporteraient encore un vrai changement (leur contenu mergé n'a jamais été committé sur cette branche).
...
## 2026-09-18 (nuit, suite) — Cinq nouveautés : simulateur prop firm, heatmap, portefeuille (Labo), exposition globale, page « Niveaux »

Esdras a demandé de choisir dans une liste d'idées : « 1, 2, 3, 6, 7 ». Tout est livré avec les mêmes garde-fous que le reste (tests unitaires sur les fonctions pures, contrôle en navigateur ordinateur + 390 px, mesure mémoire/CPU des calculs).

- **Labo — Analyses avancées** (`public/lab.html`, `src/backtest/labAnalytics.js`, routes `POST /api/lab/analyze/{challenge,heatmap,portfolio}`, ops du thread `analyzeChallenge/analyzeHeatmap/analyzePortfolio` dans `labWorker.js`) :
  1. *Simulateur prop firm* : bootstrap **par jour de trading** (un jour = ses trades ensemble, pour que la règle de perte journalière voie de vrais paquets) contre les règles d'un challenge, 4 000 tirages. Les règles se préremplissent depuis `src/propFirms` (exposées dans `/api/lab/meta` → `propPrograms`). Statique ou drawdown suiveur fin de journée ; jours minimum ; durée max. Vérifié contre la théorie (marche équitable ±10 % : 50,6 % de réussite pour 50 % attendu). Limite dite à l'écran : solde mesuré sur trades clôturés (pas de perte latente).
  2. *Heatmap jour × heure* (heure moteur) : une case n'est « fiable » que si ≥ 10 trades en entraînement ET en test et positive des deux côtés ; l'écran donne le repère « hasard pur = 1 case sur 4 » pour ne pas se laisser séduire par ~100 cases.
  3. *Portefeuille* (2 à 6 stratégies) : corrélation des R journaliers (sur les jours où au moins l'une des deux trade, sinon les jours vides communs gonflent la corrélation), equity et drawdown combinés, gain sur le drawdown, positions simultanées max, moitiés entraînement/test.
  Mesuré : un portefeuille de 6 stratégies lourdes faisait sauter le plafond de 160 Mo du thread sur EURUSD (les trades bruts portent tout le signal) ; les trades sont maintenant réduits à `{entryTime, exitTime, rMultiple}` dans le thread. Testé sur les 11 jeux du projet sans erreur.
- **Exposition globale** (`public/riskConcentration.js`, bloc « Concentration du risque » dans le Risk Command Center) : le bot ne plafonne pas les positions ouvertes simultanées (décision actée, voir plus haut) ; ce bloc dit quand deux positions corrélées vont dans le même sens (US100/US500/GER40 ; EURUSD/XAUUSD « anti-dollar »), compte les ordres en attente comme risque potentiel estimé (1 trade), et compare le risque aux stops à la marge de perte du jour et au plancher de drawdown. Script sans export, testé via `vm`.
- **Page Niveaux** (`public/niveaux.html`, `src/backtest/dailyLevels.js`, `GET /api/levels` sur les deux routeurs) : extrêmes du jour / de la veille (PDH, PDL, 50 %) / de la semaine dernière (PWH, PWL), ouverture de minuit NY, range asiatique, CBDR et projections ±2σ, gaps NWOG/NDOG, réutilisant les fonctions de production (`computeAsianRanges`, `detectNwogEvents`…), plus une frise 24 h des sessions en heure de New York. Jours/semaines = vrais jours calendaires NY (DST). Un jour « partiel » (réouverture du dimanche soir) ne remplace jamais « la veille ». **Partage la boucle d'événements avec le bot** : seuls les 15 derniers jours sont utilisés et le résultat est mis en cache par bougie fermée (le prix live n'ajoute que des distances). Lien « Niveaux » ajouté à la navigation de toutes les pages.

## 2026-09-19 — Challenge multi-stratégies avec les garde-fous du bot, et marges d'erreur partout dans le Labo

Premier lot de la liste choisie par Esdras (voir la mémoire « roadmap 2026-09-18 »).

- **Challenge avec plusieurs stratégies** (`labAnalytics.js` : `simulateMultiChallenge`, `applyGuardrailsToDay`, `normalizeGuardrails` ; onglet « Simulateur prop firm » du Labo, section repliable). On coche 2 à 6 stratégies (poids de risque par stratégie, 0 à 5) et leurs trades sont d'abord filtrés comme le bot le ferait vraiment : **N trades CLÔTURÉS par jour** (les ouverts ne comptent pas, sémantique actée), **pause après une perte** mesurée depuis la clôture de cette perte, **perte journalière du bot** sur les trades clôturés, **une seule position ouverte par symbole** (netting de `LiveStrategyEngine`). Défauts = `CONFIG.guardrails` (3 / 30 min / 2 %). Tout ne dépend que des trades du jour, donc le filtre est décidé une fois par jour et non à chaque tirage. Exemple mesuré US100 : 3 stratégies, 32 931 signaux proposés → 10 820 pris (33 %) ; réussite du challenge 23 % sans garde-fous contre 65 % avec (les garde-fous limitent l'exposition d'un ensemble à espérance négative — ce n'est pas un edge). Le Monte Carlo a été factorisé (`runChallengeSimulation`), `simulateChallenge` est inchangé.
- **Marges d'erreur** (`expectancyStats`) : chaque espérance affichée a son intervalle à 95 % et, si elle n'est pas distinguable de zéro, le nombre de trades qu'il faudrait de plus pour conclure (« aucun nombre de trades ne la confirmera » quand l'espérance est négative). Affiché dans le backtest, les deux comparateurs (colonne dédiée + « ⚠ non significatif » à côté d'un verdict « tient »), le simulateur et le portefeuille. Limite dite : trades traités comme indépendants, intervalle un peu optimiste ; en dessous de 10 trades, pas d'intervalle.

## 2026-09-19 (suite) — « Ma stratégie live » : analyse complète dans un menu replié du Labo

Demande d'Esdras : « me coder ma propre stratégie qui est live … tous les statistiques possibles … plusieurs options de combinaison … pas dans un endroit visible mais dans un menu où je clique et ça apparaît ». → Carte **repliée par défaut** dans le Labo (`<details id="live-card">`, « Ma stratégie live — analyse complète »).

- **Ce qui tourne** (`src/backtest/liveFvgRunner.js`) : le FVG filtré réellement tradé par le bot (`CONFIG.fvg.perSymbol` : biais HTF + structure + session + balayage de liquidité, multi-contact, R:R fixe), construit avec **exactement** le même moteur que `LiveStrategyEngine._buildFvgEngine()` (`MultiTouchFvgEngine` + `buildMultiTouchFilterPredicate`, sinon `buildFilteredEngine`), coûts de spread appliqués, même coupure entraînement/test. Seuls US100, US500 et XAUUSD ont une config FVG live (les autres symboles répondent 400 avec ce message). Vérifié contre le résumé indépendant du moteur (`summarizeTrades`) sur US100 : 1215 trades, espérance 0,8423R des deux côtés, total 1023,40R, profit factor 2,124, drawdown 32,76R vs 32,77R (arrondi des R à 4 décimales dans la charge utile).
- **Combinaisons** : 18 variantes prêtes (« la config live sauf… » : sans biais HTF / structure / session / balayage, contact unique ou multi-contact, objectifs 1:2 à 1:6, stop au bord ou au swing, fenêtres 8–12, 10–11, 7–10, FVG brut) + un formulaire « construire ma propre combinaison » (validé côté serveur, `normalizeCustomOverrides`). 8 combinaisons au plus par lancement ; la config live est toujours lancée en référence. Une comparaison finale les juge de la même façon et rappelle combien ont été essayées (biais des essais multiples).
- **Statistiques** (`src/shared/tradeStats.js`, module unique servi aussi au navigateur sur `/shared/tradeStats.js`, donc les chiffres de la page viennent du code que les tests couvrent ; `expectancyStats` y a déménagé, `labAnalytics` le ré-exporte) : ~25 indicateurs (espérance + marge d'erreur, profit factor, gain/perte moyens, ratio, médiane, meilleur/pire, Sharpe et Sortino par trade, Kelly indicatif, séries max, drawdown max/actuel/durée sous l'eau, durée moyenne, trades par mois, probabilité de 3/5/8 pertes de suite), entraînement contre test, equity, sous l'eau, histogramme, espérance glissante sur 50 trades, tableau R par mois × année, distribution des séries, pires/meilleurs trades, export CSV.
- **Découper / croiser** : 11 dimensions (année, trimestre, mois, jour, heure d'entrée **New York** DST-aware, sens, issue, durée, taille du stop en terciles, trade précédent, rang dans la journée), tableau par dimension avec barre et marge d'erreur, et croisement de deux dimensions au choix avec 4 mesures (espérance, réussite, total R, nombre) et 8 croisements prédéfinis. Tout est calculé dans le navigateur à partir de la liste de trades : aucun aller-retour serveur pour changer de découpage.
- Mesuré : 8 combinaisons sur US100 = 8 s en local (≈ 1 min attendue en prod), charge utile 4,7 Mo (la variante « FVG brut » pèse ~17 000 trades).

## 2026-09-19 (suite) — Calque « Sessions » sur le graphique, modifiable comme les zones FVG et les signaux

Demande d'Esdras : voir les sessions sur les graphiques, activables/désactivables comme FVG et signaux, et modifiables.

- **`public/chart.html`** : nouveau calque « Sessions » dans le panneau Calques, avec **une case par session** (CBDR 14–20, Range asiatique 20–24, Killzone Londres 2–5, Session New York AM 8–12 = la fenêtre du bot sur US100/XAUUSD, Silver Bullet 10–11) et **deux champs d'heures par session** pour la modifier (validation : début < fin, 0–24 ; une fenêtre invalide est ignorée et l'ancienne valeur revient) + « Réinitialiser les sessions ». Les bandes sont dessinées **derrière les bougies** par une primitive de série (`SessionsPrimitive`, `zOrder: 'bottom'`) ; heures = heure de New York **DST-aware** (`nyLocalToUtcMs`, vérifié : 8 h NY = 12:00Z en juillet, 13:00Z en janvier, 12:00Z le jour du passage à l'heure d'été 2020-03-08). Un jour sans bougie dans la fenêtre (week-end) ne dessine rien. **M15 et H1 seulement** : sur H4/D1 une session est plus étroite qu'une bougie, la note « Sessions visibles en M15 et H1 seulement » s'affiche. Choix mémorisés dans `localStorage` (`chartSessionState`, confort par navigateur, try/catch).
- **Une seule liste de fenêtres** : `GET /api/sessions` sert `SESSION_WINDOWS` (`dailyLevels.js`, qui a gagné la fenêtre `nyam` 8–12), utilisée par le graphique ET par la frise de la page Niveaux — une fenêtre ne peut pas différer d'une page à l'autre.
- **Mobile** : les pages `chart`, `journal` et `accounts` débordaient à 390 px depuis l'ajout du sixième lien de navigation (« Niveaux ») ; la règle `.topnav { overflow-x: auto }` d'`index.html` y a été copiée, et le groupe de boutons de symboles du graphique passe à la ligne (`flex-wrap`). Vérifié : plus aucun débordement horizontal sur les six pages.

## 2026-09-19 (suite) — Contexte historique sous chaque signal FVG (roadmap n° 4, « score de qualité »)

Sous chaque signal FVG actionnable du dashboard : « Historique de signaux comparables — 11h New York · Achat · stop large : 103 trades, +0,08R en moyenne (marge d'erreur −0,37 à +0,52R), 19 % de réussite · avant 2024 +0,14R (87), depuis −0,26R (16) · ⚠ en dessous de la moyenne de la stratégie (+0,84R) ».

**Choix délibéré : pas de « note sur 100 ».** Un score ajusté sur 17 ans de trades se sur-apprend facilement et donnerait une précision fictive. À la place, une **consultation empirique** (`signalContext` dans `src/shared/tradeStats.js`) : l'historique de la stratégie live (config live, coûts inclus) restreint du plus précis au plus général — heure New York + sens + taille du stop (tercile), puis heure + sens, heure, sens, tout — et on s'arrête au premier groupe qui a au moins 30 trades. Verdict « au-dessus / en dessous » **seulement si la marge d'erreur à 95 % du groupe dépasse la moyenne de la stratégie**, sinon « pas distinguable » ; le groupe est aussi montré avant/depuis 2024. Le texte rappelle que parmi tous les groupes possibles certains paraissent bons ou mauvais par pur hasard.

Technique : `GET /api/signal-context` (compte-agnostique) ; l'historique est calculé **une fois par symbole dans le thread du Labo** (2,4 s la première fois pour US100, 21 ms ensuite) et gardé 12 h, jamais sur la boucle d'événements du bot ; l'heure du signal arrive en UTC réel et est ramenée en heure moteur (−5 h fixe). Seuls US100/US500/XAUUSD (les symboles qui ont une config FVG) ; les autres n'affichent aucune ligne. Côté page, la réponse est mise en cache par signal (elle ne change pas) : vérifié 2 requêtes pour 2 signaux malgré les nombreux rafraîchissements. Piège de test rencontré : le flux SSE de la page remplace les signaux simulés au bout de quelques secondes (artefact du test, pas un défaut — on l'abat dans le test).

## 2026-09-19 (suite) — Débrief automatique de chaque trade clôturé, et « mentor » Claude (roadmap n° 6 + journal auto-annoté)

Dans le journal, chaque trade clôturé a un bouton **« Débrief automatique »** (`POST /api/trade-debrief`, `src/backtest/tradeDebrief.js`). C'est le « journal qui s'annote seul » : des **faits** en phrases courtes — les sessions dans lesquelles l'entrée est tombée (heure New York), la vérification contre la fenêtre de session **du bot** pour ce symbole (✅ dedans / ⚠ dehors), les niveaux clés à moins de 0,25 % de l'entrée (veille, semaine, minuit, ranges…), jusqu'où le prix est allé **pour et contre** le trade pendant qu'il était ouvert (avec une mise en garde sans en faire une règle quand un perdant a été bien dans le vert), le résultat en R, et l'historique des signaux comparables (même consultation empirique que sous les signaux du dashboard, verdict par marge d'erreur). Aucun modèle n'intervient ici. Un trade plus ancien que l'historique de bougies gardé par le bot (~90 jours) l'indique au lieu d'inventer un contexte.

**Mentor** (`POST /api/trade-mentor`, `answerTradeMentor` dans `chatAssistant.js`) : bouton « Avis du mentor » qui envoie au modèle (Haiku 4.5, comme le chat) **uniquement le débrief factuel** et lui demande ≤ 90 mots : une chose bien faite, une chose à regarder, un rappel qu'un trade est un échantillon minuscule ; aucun chiffre inventé, aucun conseil financier, aucun changement de règle sur un seul trade. La sortie du modèle est insérée en `textContent`, jamais en HTML.
⚠ **Non actif en production** : `ANTHROPIC_API_KEY` n'est pas configurée sur Render (vérifié 2026-09-19 : `POST /api/chat` répond 503 « ANTHROPIC_API_KEY manquant »). Sans clé, le bouton du mentor est remplacé par une phrase qui l'explique et le débrief factuel fonctionne quand même. Le chemin avec clé n'est **pas testé de bout en bout** (seuls la construction du prompt et le refus sans clé le sont). Pour l'activer : variable d'environnement `ANTHROPIC_API_KEY` dans Render.

## 2026-09-19 (suite) — Salle de contrôle plein écran (roadmap n° 7)

Nouvelle page `public/salle.html` (lien « Salle » dans la navigation de toutes les pages), pensée pour rester ouverte sur un deuxième écran : horloges New York et locale, frise des sessions avec compte à rebours, une tuile par symbole (prix en grand, niveau clé le plus proche au-dessus et en dessous avec distance en %, position ouverte avec son **R en direct** calculé depuis le stop, pause après perte, contour bleu pulsant quand un signal est actionnable), jauges des garde-fous (trades clôturés du jour, perte du jour, marge avant le plancher de drawdown, « nouveaux trades autorisés/bloqués » avec les raisons), signaux actionnables et dernières alertes. Bouton **Plein écran** (Fullscreen API) et **Son** : un bip discret à chaque nouveau signal ou nouvelle alerte, désactivé par défaut et mémorisé (les navigateurs refusent le son avant un clic, d'où le bouton). L'animation respecte `prefers-reduced-motion`. **Rien de nouveau côté serveur** : elle n'utilise que les routes existantes (`/api/status`, `/api/signals`, `/api/levels`, `/api/alerts`, `/api/sessions`), interroge à des rythmes différents (4 s / 8 s / 20 s / 10 s) et se met en pause quand l'onglet est caché, pour ne pas peser sur le bot. Vérifié avec des données simulées (positions, signaux, alertes), ordinateur et 390 px, sans débordement ; le bip sur nouveau signal a été constaté (contexte audio créé après l'apparition d'un second signal).

## 2026-09-19 (suite) — Lego de stratégies (roadmap n° 3) et cimetière des idées (C)

**Lego** (Labo, carte repliée « Lego — construire ma propre stratégie », `src/backtest/legoStrategy.js`, `POST /api/lab/lego`) : une **recette** = un déclencheur (un des 21 mécanismes du Labo : ses entrées et ses stops) + des filtres (fenêtre de session en heure New York, jours de la semaine, sens, biais de la timeframe supérieure H1/H4 × EMA 20/50/200, structure BOS, balayage de liquidité) + un objectif R:R optionnel + les garde-fous du bot (mêmes règles que le simulateur de challenge). Jugée comme tout le reste (coûts, coupure entraînement/test, statistiques complètes) et **toujours comparée au déclencheur seul**. Les filtres de marché réutilisent les constructeurs des filtres FVG live (`buildMultiTouchFilterPredicate`), évalués à la bougie d'entrée. `resimulateExit` rejoue la sortie avec un autre R:R (stop gagnant en cas d'égalité dans une bougie, délai de 480 bougies). **Limites écrites sur la page et ici** : (1) les filtres sont appliqués *après coup* — un trade retiré ne libère pas sa place pour un autre signal, contrairement à un filtre codé dans le moteur (ceux du FVG live le sont) ; (2) un autre R:R remplace la sortie propre du mécanisme (sans conséquence pour les mécanismes à objectif fixe 1:3, c'est une autre stratégie pour les suiveurs de tendance/à niveau-cible : DMI/MACD n'ont pas de cible, VWAP ancré et Midnight Open visent un niveau) ; (3) chaque recette essayée est une chance de plus de trouver un faux gagnant. Exemple mesuré US100 : Asian Range Breakout + session 0h–5h + mar/mer/jeu + biais H4 EMA200 + 1:4 + garde-fous → 2 513 signaux, 766 après filtres, 753 trades finaux.
Les résultats du Lego et de « Ma stratégie live » s'affichent dans une **carte commune « Analyse détaillée »** (masquée tant qu'on n'a rien lancé), qui a remplacé la zone de résultats de la carte live.
...
- FAIT et testé (`test/labAnalytics.test.js`, 33 tests) : le Monte Carlo accepte un **état de départ** (`state: {balance, peakEod}` en % du solde initial) et produit un **cône** d'equity (`cone: N` jours → percentiles 5/25/50/75/95 par jour) ; le plancher `trailing-eod` est maintenant **multiplicatif** comme `GuardrailEngine._overallDrawdownFloor()` (pic de fin de journée × (1 − dd %)), avant c'était un nombre de points fixe (écart faux de quelques dixièmes) ; le « une position ouverte par symbole » peut être appliqué **par symbole** (`perSymbol: true`).
...
**Décision d'Esdras (2026-09-19) : le challenge HaitiForex est abandonné** (« it isn't worth it »). Ne plus y travailler : ni le câblage de la fermeture forcée à 16h NY, ni la modélisation du plafond de gain dans la carte « Probabilité de finir le challenge ». Le code et les scripts existants (`src/propFirms/haitiforex.js`, `dailyGainCapUsd` dans `GuardrailEngine`, `scripts/runHaitiForex*.js`) restent en place, inertes tant qu'aucun compte n'utilise ce programme. Seule réserve connue si un compte HaitiForex était un jour configuré : la carte d'issue de challenge ne modélise pas le plafond de gain journalier (résultat trop optimiste) — à traiter à ce moment-là, pas avant.
...
**Fait juste après (2026-09-19, même session)** : le challenge FTMO 1-Step rejoué avec le correctif (`scripts/runFtmo1StepFullComboAccountImpact.js`, cible +10% RÉELLEMENT active cette fois, pas désactivée comme dans la comparaison ci-dessus) — **le challenge passe toujours, et plus proprement qu'avant** : 156 trades, WR 25.6%, solde final $11134.76 (**+11.35%**), cible +10% atteinte le **2026-04-17** (2 des 7 mois, pas toute la fenêtre), drawdown 10% **jamais franchi** sur cette fenêtre (contrairement au passage "activité continue" ci-dessus, qui lui n'arrête jamais de trader même après avoir gagné et finit par franchir le drawdown en septembre — ce n'est PAS ce qu'un vrai compte FTMO aurait fait, puisque personne ne continue de trader un challenge déjà gagné). Le correctif ne change donc pas la conclusion "ça passe" — au contraire, il l'explique mieux : la fausse impression de "+70% sur 7 mois complets" masquait le fait que le combo gagne le challenge tôt (avril) et devrait s'arrêter là.

**Convergence avec le thread ci-dessous** : la session qui a construit le Simulateur de marché rejouable a trouvé EXACTEMENT le même problème, indépendamment, en comparant ses propres résultats à ceux du backtest (voir "Résultats du simulateur ≠ R du backtest, et c'est voulu", point 2) — mais a choisi de le corriger seulement dans son propre courtier simulé (`replayBroker.js`, avec une option "comme le backtest" pour reproduire l'ancien comportement à des fins de comparaison), sans toucher à `LiveStrategyEngine` lui-même. Le correctif ci-dessus, lui, corrige la source (le moteur central) — donc désormais le "R brut du backtest" que cette page affiche pour comparaison va changer lui aussi (probablement se rapprocher du chiffre du simulateur, puisque c'est la même cause qui expliquait l'écart).

## 2026-09-19 (suite) — Simulateur de marché rejouable (`/simulateur.html`, lien « Simulateur » partout)

Demande d'Esdras : rejouer le marché avec (1) les 8 stratégies du bot en une seule option, (2) n'importe quelle autre stratégie, (3) un vrai panneau de trading « comme les simulateurs de forex », pour VOIR comment les stratégies apparaissent et se jouent ; changement de timeframe ; **les news sur le graphique** ; **un endroit pour importer de nouvelles paires**.

- **Rejeu** : `POST /api/lab/replay` (`{symbol, from?, days 1-120, strategyIds, news}`) → bougies M15 (400 bougies de contexte avant la date de départ), trades des stratégies dans la fenêtre, annonces. Les stratégies tournent sur tout l'historique (leurs signaux ont besoin de l'échauffement) dans le thread du Labo ; seuls les trades qui touchent la fenêtre sont renvoyés. La page **n'affiche un trade qu'à partir de son heure d'entrée et son résultat qu'à partir de sa sortie** (pas de « futur » visible). Heures en New York, DST géré ; le passé dépend du fuseau moteur (−5 h fixe) converti côté serveur.
- **« Les 8 du bot »** (`src/backtest/replaySignals.js`, `BOT_MECHANISMS`) : FVG filtré (config live exacte), Divergence US100/US500, NWOG (US100 long seulement comme en live), Judas Swing, Weekly Sweep, Breaker Block, Silver Bullet, CBDR — chacun avec **les symboles et le R:R de `CONFIG`** ; sur une paire où le bot ne trade pas un mécanisme, celui-ci est signalé « non applicable », jamais inventé. La logique des candidats Divergence est une copie de `LiveStrategyEngine._computeDivergenceCandidates` **verrouillée par un test de parité contre la vraie classe**. Le partenaire de la Divergence est lu en **horaire streamé** (`loadResampledFromCsv`, résultat identique à `resampleCandles(loadCandlesFromCsv())`, testé) car deux séries M15 complètes ne tiennent pas dans le thread plafonné à 160 Mo. Les 21 stratégies du Labo sont aussi disponibles (réglages par défaut).
- **Courtier simulé** (`src/shared/replayBroker.js`, pur, servi au navigateur et testé, 16 tests) : le graphique montre le **bid** ; achat au ask (bid + spread), sortie au bid, spread payé une fois par aller-retour comme les coûts du Labo. Ordres au marché / limite / stop (côté du marché imposé), stop, objectif, **stop suiveur**, break-even, fermeture partielle, tout fermer, taille en unités **ou au risque (% du solde)**, R calculé sur les trades qui avaient un stop. Règles prudentes : une position n'est jugée qu'à partir de la bougie SUIVANT son ouverture ; si une bougie touche stop et objectif, **le stop gagne** ; un stop traversé par un gap se remplit à l'ouverture (pire). Bouton « Prendre » sur chaque signal apparu (reprend son stop/objectif). Limites écrites sur la page : bougies M15 (pas de tick), P&L en points × unités sans conversion de devise (exact pour les indices, approché pour le forex).
- **News** (`src/backtest/newsCalendar.js`) : les annonces « rouges » déjà sourcées du projet (`newsEvents.js` : CPI, NFP, FOMC, PIB, PCE, ventes au détail US, BCE), maintenant **nommées** ; la segmentation en groupes est verrouillée par des tests d'ancrage sur des dates réelles. **Couverture 2024–2025 seulement** (dit sur la page) ; hors de cette période la liste est vide, jamais devinée. Lignes verticales + libellé sur le graphique, liste « à venir / passées », bouton « → news ».
- **Import de paires** : carte « Importer de nouvelles paires » dans la page, même route/même validations que le Labo (token admin, fichier par année, 40 Mo max) ; la paire importée apparaît dans la liste et se rejoue tout de suite. Testé de bout en bout (2 fichiers M1 → EURGBP, rejeu avec MACD/OTE, mauvais token refusé, message clair que le bot ne trade rien sur cette paire). Stockage toujours temporaire (disque Render).
- **Mesuré** : rejeu de la période avec les 8 du bot + les 21 du Labo : 10 à 17 s en local (≈ 75–130 s en prod à ×7,6, sous le délai de 420 s). Un premier essai faisait sauter le plafond mémoire du thread sur EURUSD (les trades de 21 stratégies mis en cache) ; on ne garde plus en cache que les listes de ≤ 4 000 trades.
- **Pas encore fait** : couches de sessions sur ce graphique (existent sur `chart.html`), déplacement des stops à la souris, tick par tick.

## 2026-09-19 (suite 2) — Simulateur v2 : bot seul, multi-paires, M1/M5, retour en arrière, lots + conversion

Demande d'Esdras : rejouer les combos « seuls » (le bot prend les trades tout seul, P&L en direct, « Prendre » désactivé), dates de début/fin + solde de départ, retour en arrière / stop / zoom, et les autres paires en onglets, sélectionnables pendant le rejeu.

- **Mode « Le bot seul »** (`src/shared/replayAuto.js`, pur, 10 tests) : chaque trade des stratégies cochées est pris à son prix d'entrée backtest (achat au ask), dimensionné au risque % du solde, résolu par les stops/objectifs du courtier ou fermé à la sortie backtest ; garde-fous du bot optionnels (3 trades fermés/jour, pause 30 min après perte, -2 %/jour, 1 position par paire). Les trades antérieurs au départ (ou à l'ajout d'une paire) ne sont jamais pris. « Prendre » et le panneau d'ordres sont grisés dans ce mode.
- **Multi-paires** : jusqu'à 4 paires ; la première est l'horloge, les autres suivent au même instant ; onglets avec prix et P&L latent ; « Ajouter une paire » fonctionne pendant le rejeu (`POST /api/lab/replay` avec `fromMs`). Courtier : `onCandle(acct, candle, symbol)`, spreads/cours de conversion par symbole.
- **Retour en arrière exact** : instantanés (~400) + journal des actions de l'utilisateur ; on restaure l'instantané précédent puis on rejoue en silence (déterministe). Testé : avancer/reculer/avancer redonne exactement le même solde et les mêmes trades.
- **M1 conservé à l'import** (`src/backtest/m1Store.js`, fichier binaire 20 octets/minute, fusion en flux, plafond disque 400 Mo) ; rejeu en M1/M5 pour ces jeux (M1 ≤ 30 j). Lots + taille de contrat (`instrumentSpecs.js`) et conversion du P&L en dollars quand une paire de conversion existe dans les données (sinon dit explicitement).
...
## 2026-09-19 (suite 4) — FTMO 1-Step sur les 7 mois réels, règlement réaliste ; filtre du bot dans le simulateur

- **(Mis à jour après le correctif du moteur par une autre session : les 3 conventions donnent maintenant le même résultat, 2 défis réussis / 1 échec / 1 en cours ; le tableau 5/3/0 ci-dessous date d avant ce correctif.)** **Nouveau `scripts/runFtmo1StepRealisticCycles.js`** (rapport : `data/real-data-2026-02-to-09/ftmo-1step-realistic-cycles.md`) : mêmes trades que `runComboVsPyramidAccountImpact.js` (347 trades, 17 021 $ reproduits), puis cycles FTMO 1-Step (+10 %, perte jour 3 %, perte max 10 % trailing EOD, 10 000 $ par tentative, 0,5 % de risque) sous 3 conventions de règlement de la bougie d'entrée : « script » (ancienne, bougie d'entrée sautée) **5 réussis / 1 échec** ; « mi-chemin » (seuls les trades entrés à l'ouverture corrigés, certains) **3 / 1** ; « prudent » (tout stop touché dans la bougie d'entrée = perte) **0 / 3**. Le vrai résultat est entre « mi-chemin » et « prudent ». Les anciens scripts de cycles sautent la bougie d'entrée ET (pour 3 d'entre eux) ne décalent pas de -5 h.
- **Simulateur** : le bot seul applique maintenant le filtre du vrai bot « stop >= 3x le spread » (`MIN_STOP_SPREAD_MULTIPLE`, comme `liveStrategyEngine`) ; avant, il prenait des trades à stop minuscule que le vrai bot refuse. Même fenêtre, 0,5 %, garde-fous : ~11 080 $ (+11 %), pire baisse 17 %. Option « fermer avant week-end / rollover » ajoutée (test : aucun gain fiable). Données réelles décalées de -5 h côté serveur ; le trade du bot est jugé sur sa propre bougie, stop rempli au prix du stop.

## 2026-09-20 — Où placer le stop ? Test « stop plancher en ATR » sur les 7 mois réels
...
## 2026-09-20 (suite 3) — Plan FTMO 1-Step : politique de risque + configuration recommandée sur les 7 mois de 2026

`scripts/runFtmoPlan.js` (+ `scripts/lib/stopFloorCore.js`, cœur partagé : trades bruts -> re-résolution avec plancher ATR par mécanisme, garde-fous simplifiés, cycles FTMO) -> `data/backtest-input/ftmo-plan-2010-2026.md`. Configuration recommandée = 13 mécanismes (US500 Silver et US100 CBDR retirés) avec plancher par mécanisme (`RECOMMENDED` dans le lib, choisi sur <= 2023 uniquement).
- **Politique de risque** (portefeuille recommandé, tentatives FTMO 1-Step de 10 000 $) : taux de réussite 0,25 % 76 % (médiane 139 j), 0,50 % 60 % (39 j), 0,75 % 57 % (20 j), 1,00 % 55 % (12 j) ; 0,5 % puis 0,25 % sous -4 % : 64 %. Le bot d'aujourd'hui (15 mécanismes, stops d'origine, 0,5 %) : 43 % (2010-2025), 50 % (2024-25). FTMO 1-Step n'a pas de limite de temps : réduire le risque augmente les chances au prix de la durée.
- **2026 (7 mois réels, hors échantillon)** : aujourd'hui +18,6 % (baisse 19 %, FTMO 3/3/1) ; recommandé +23,7 % (baisse 11 %, 3/2/1) ; seulement les 6 « GARDER » +2,0 % (le sous-ensemble ne tient pas). Verdicts par mécanisme instables en 2026 (US100 FVG -11,6R, US500 FVG -7,9R alors que US500 Weekly +15R, XAUUSD FVG +12,8R, EURUSD Judas +5R) : ne pas trop élaguer sur des verdicts individuels ; l'amélioration tient au niveau du portefeuille. Nombres de trades différents du moteur live (429 vs 347) : trades bruts des modules, pas `LiveStrategyEngine`.
...
Esdras a donné le jeton admin (jamais écrit dans le dépôt) ; `GET .../admin/export-candles?timeframe=M1&days=120` a fourni ~109 500 bougies d'1 minute par paire (US100, US500, GER40, XAUUSD, EURUSD), du 19 mai au 18 sept. 2026, en UTC réel -> `data/real-m1/*.csv` (27 Mo). Le broker garde au moins 120 jours de M1 (plafond de la route ; à relever pour remonter à février).
`scripts/runM1Truth.js` (+ `resolver` optionnel dans `scripts/lib/stopFloorCore.js`) -> `data/real-m1/m1-truth-report.md` : mêmes trades, réglés en M15 prudent puis minute par minute.
- **Sur les trades qui se remplissent, M1 et M15 prudent donnent la MÊME issue (158/158)** : l'incertitude « stop ou objectif en premier dans une bougie M15 » est levée, la règle prudente utilisée jusqu'ici n'était pas biaisée.
- **Ce qui change les totaux** : des entrées de backtest (~10 %, 41 sur ~420 ; 37 ne touchent JAMAIS leur prix d'entrée dans les 8 bougies suivantes, surtout FVG/Silver/Breaker/Divergence) ne sont pas des prix atteignables dans les vraies données du broker. Écartées en M1, le classement des configurations s'inverse : aujourd'hui M15 -5,4 % -> M1 +8,0 % ; recommandé M15 +14,8 % -> M1 +4,8 % (~4 mois, ~200 trades). Donc **l'avantage de la configuration recommandée n'est PAS confirmé sur cette fenêtre** ; les deux sont légèrement positifs (+5 à +8 % en 4 mois, baisse ~11 %). A investiguer : d'où viennent ces prix d'entrée inatteignables (modules de backtest vs bougies bid du broker) - probable source d'optimisme de tout le backtest.

**CORRECTION (même jour) de l'entrée précédente** : les « entrées inatteignables » n'étaient PAS un défaut du backtest : 36 des 41 tombaient dans des bougies M15 où il manque des minutes M1. Une bougie M1 n'existe que s'il y a eu un tick : les minutes calmes (GER40 623 minutes manquantes, EURUSD 987, US500 67, US100 0, XAUUSD 4) n'ont pas de barre, le prix est simplement resté où il était. Règle corrigée dans `runM1Truth.js` : une entrée à l'ouverture de la bougie se remplit à sa première minute disponible ; seules 4-5 entrées limite ne sont jamais touchées. Résultat corrigé (19 mai -> 18 sept. 2026, 10 000 $, 0,5 %) : **aujourd'hui M1 exact +10,8 % (baisse 11 %, FTMO 2 réussis / 2 échoués / 1 en cours) ; recommandé M1 exact +8,8 % (baisse 9 %, FTMO 1 / 0 / 1)**. Sur les MÊMES trades : recommandé 164/164 issues identiques M15 vs M1 (R +25,1 des deux côtés, donc robuste à l'ordre stop/objectif) ; aujourd'hui 215/222 identiques, R +0,7 en M15 prudent contre +23,7 en M1 : avec les stops minuscules d'origine, la règle prudente M15 sous-estime de ~23R (7 trades où l'objectif est en fait touché avant le stop dans la bougie). Conclusion : sur 4 mois réels les deux configurations rapportent ~+9 à +11 % ; le plancher ATR ne gagne pas plus mais rend le résultat fiable (indépendant de l'ordre intrabougie) avec une baisse un peu plus faible.
