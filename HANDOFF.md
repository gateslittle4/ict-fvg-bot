...
**1) Récidive confirmée du gap de double-position (netting)** — déjà vu une fois à 02h00 (sens opposés), reproduit une 2e fois à 09h11 (même sens, achat) : 2 positions RÉELLES BTCUSD ouvertes simultanément (41549468 @ 08:30, 41550320 @ 09:11). Cause : `LiveStrategyEngine.openPositions` est une croyance SIMULÉE (voir plus haut, "belief-based netting") — sa propre clôture simulée (`_resolveOpenPosition`, contre les bougies) peut se produire AVANT que la position réelle ne se ferme vraiment chez le courtier, ce qui laisse le netting croire la voie libre et autoriser une 2e position réelle par-dessus la 1ère encore ouverte. Les fixes de cette nuit (auto-clear sur `orderId` null, auto-clear sur fermeture réelle confirmée) couvrent des cas différents (croyance qui reste bloquée APRÈS la réalité) — pas celui-ci (croyance qui se libère AVANT la réalité). Posé à Esdras à 02h13 ("tu veux que je m'y attaque maintenant, ou on garde ça aussi pour la prochaine session ?"), sans réponse reçue avant la fin de cette session de surveillance.

**2) NOUVEAU : `GuardrailEngine` ne survit pas à un redémarrage** — `this.trades` (utilisé pour `tradesToday`/`maxTradesPerDay` et `dailyPnl`/`dailyLossLimitPct`) vit uniquement en mémoire (`src/engines/guardrailEngine.js`), sans aucune reconstruction depuis l'historique réel au boot — contrairement à `LiveStrategyEngine.openPositions`, qui LUI est reconstruit via `warmUp()`, et contrairement à `maxDrawdownPct` (overall, pas quotidien), documenté plus haut comme "persistant, ne se réinitialise pas au changement de jour". Vérifié en direct : deux redémarrages via `/api/admin/restart` (09h31 et 09h34, visiblement volontaires — pas un crash, rien d'anormal dans les logs autour) ont remis `tradesToday` à 0 dans `/api/status`, alors que plusieurs trades BTCUSD réels avaient déjà eu lieu plus tôt le même jour (07h44, 08h30, 09h11). Autrement dit : **`maxTradesPerDay` et `dailyLossLimitPct` peuvent être contournés en redémarrant le service plusieurs fois dans la même journée** (via `/api/admin/restart`, ou un vrai crash/redéploiement). Pas exploité ce soir (aucun signe de mauvaise intention, juste un redémarrage admin normal), mais c'est un vrai trou de protection à combler, probablement en reconstruisant `this.trades` au boot depuis l'historique réel du courtier (même principe que `warmUp()`/`_loadClosedDeals`, déjà utilisé ailleurs pour un besoin similaire) plutôt qu'en ajoutant une persistance disque à part.

**Fichiers concernés** : `src/engines/guardrailEngine.js`, `src/liveStrategyEngine.js` (netting), `src/dataSources/cTraderDataSource.js` (`_clearStaleBeliefsAgainstBroker`, `_loadClosedDeals`, point d'appel `/api/admin/restart`).
