// intradayMomentum.js (src/backtest/ depuis 2026-09-24 : partagé par les études ET le bot live, voir src/intradayMomentumEngine.js)
// - règles des trois stratégies de momentum intraday pré-enregistrées dans
...
/** A - partie décision (partagée avec le bot live) : sens et extrêmes de la première bougie de 5 min (barres k <= 4). Null si pas de barre
 * ou doji (clôture = ouverture). */
export function orbSetup(d) {
...
export function orbTrade(d, spreadAt, { rr = 10, minStopSpreads = 3 } = {}) {
  const setup = orbSetup(d);
  if (!setup) return null;
  const { long, hi, lo } = setup;
...
/** B - partie décision (partagée avec le bot live) : bornes et prix au contrôle k de la séance days[i], avec les barres k' <= k seulement.
 * TWAP (remplace le VWAP, pas de volume) = moyenne des prix typiques des barres depuis 9:30 jusqu'à k. Null s'il manque des séances. */
export function noiseCheck(days, i, k, { lookback = 14 } = {}) {
  ...
  sig /= lookback;
  let sum = 0, n = 0, px = d.open;
  for (let j = 0; j < d.k.length && d.k[j] <= k; j++) { sum += (d.h[j] + d.l[j] + d.c[j]) / 3; n++; px = d.c[j]; }
  const twap = n ? sum / n : d.open;
  return { ub: upBase * (1 + sig), lb: dnBase * (1 - sig), px, twap, sig };
}

/** B - règle de décision à un contrôle : sortie si la position repasse sous max(borne haute, TWAP) (achat) / au-dessus de
 * min(borne basse, TWAP) (vente) ; puis, si à plat, entrée au-delà d'une borne. `pos` = null | { long }. */
export function noiseDecision(pos, st) {
  let exit = false;
  if (pos) exit = pos.long ? st.px < Math.max(st.ub, st.twap) : st.px > Math.min(st.lb, st.twap);
  const flat = !pos || exit;
  const enter = flat ? (st.px > st.ub ? 'long' : st.px < st.lb ? 'short' : null) : null;
  return { exit, enter };
}
...
export function noiseTrades(days, i, spreadAt, { lookback = 14 } = {}) {
  ...
  for (const k of NOISE_CHECKS) {
    if (k > d.lastK) break;
    const st = noiseCheck(days, i, k, { lookback });
    const e = barAtOrAfter(d, k + 1);
    if (e < 0) break;
    const ob = d.o[e], s = spreadAt(ob);
    const dec = noiseDecision(pos, st);
    if (dec.exit) {
    ...
    if (dec.enter === 'long') pos = { long: true, fill: ob + s, time: d.t[e] };
    else if (dec.enter === 'short') pos = { long: false, fill: ob, time: d.t[e] };
  }
  if (pos) {
